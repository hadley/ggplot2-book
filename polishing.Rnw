...
<<polishing, echo=FALSE>>=
knitr::set_parent('knit-book.Rnw') # allows us to compile as an individual document
knitr::opts_chunk$set(out.width = "0.32\\linewidth", cache.path = "cache/polishing-", fig.path = "figure/polishing-")
...
<<themes, fig.cap="The effect of changing themes.  (Left) The default grey theme with grey background and white gridlines.  (Right) the alternative black and white theme with white background and grey gridlines.  Notice how the bars, data elements, are identical in both plots.", echo=FALSE>>=
...
<<hgram, prompt=TRUE, fig.show='asis', fig.align='left'>>=
...
\begin{itemize}
  \item \f{element_text} draws labels and headings. You can control the font \code{family}, \code{face}, \code{colour}, \code{size}, \code{hjust}, \code{vjust}, \code{angle} and \code{lineheight}. \index{Themes!labels} \indexf{element_text}
...
<<theme-text, out.width="0.32\\linewidth", fig.cap="Changing the appearance of the plot title.">>=
...
@
  ...
  \item \f{element_line} draws lines with the same options but in a slightly different way.  Make sure you match the appropriate type or you will get strange grid errors.  For these element functions you can control the \code{colour}, \code{size} and \code{linetype}.  These options are illustrated with the code and the results are shown in Figure~\ref{fig:theme-line}. \indexf{element_line}

<<theme-line, out.width="0.32\\linewidth", fig.cap="Changing the appearance of lines and segments in the plot.">>=
...
<<theme-background, out.width="0.32\\linewidth", fig.cap="Changing the appearance of the plot and panel background">>=
...
<<theme-update, fig.cap="A bar chart and scatterplot created after a new visually consistent (if ugly!) theme has been applied.">>=
...
<<geom-defaults, results='asis', echo=FALSE, eval=FALSE>>=
...
df[] <- lapply(df, function(x) paste("\\code{", x, "}", sep =""))
df2 <- ddply(df, .(Aesthetic, Default), summarise, Cap = paste(Geoms, collapse = ", "))
pander::pandoc.table(df2, caption = "Default aesthetic values for geoms.  See Appendix~\\ref{cha:specifications} for how the values are interpreted by R.")
@
...
<<layout, fig.cap="Three simple graphics we'll use to experiment with sophisticated plot layouts.">>=

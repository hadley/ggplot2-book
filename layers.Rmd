...
These generated variables can be used instead of the variables present in the original dataset. For example, the default histogram geom assigns the height of the bars to the number of observations (`count`), but if you'd prefer a more traditional histogram, you can use the density (`density`). To refer to a generated variable like density, "`stat()`" must wrap the name. This prevents confusion in case the original dataset includes a variable with the same name as a generated variable, and it makes it clear to any later reader of the code that this variable was generated by a stat. Each statistic lists the variables that it creates in its documentation. \indexc{stat} Compare the y-axes on these two plots:
...
ggplot(diamonds, aes(price)) + 
  geom_histogram(aes(y = stat(density)), binwidth = 500)
```
...
ggplot(diamonds, aes(price, colour = cut)) + 
  geom_freqpoly(aes(y = stat(density)), binwidth = 500) + 
  theme(legend.position = "none")
```

...
-   **mapping**: A set of aesthetic mappings, specified using the `aes()` function and combined with the plot defaults as described in Section \@ref(aes).
...
-   **data**: A dataset which overrides the default plot dataset.
    ...
    The requirements for data are explained in more detail in Section \@ref(data).

-   **geom**: The name of the geometric object to use to draw each observation.
    Geoms are discussed in more detail in Section \@ref(data), and Chapters \@ref(individual-geoms) and \@ref(collective-geoms) explore their use in more depth.

    Geoms can have additional arguments.
    All geoms take aesthetics as parameters.
    If you supply an aesthetic (e.g. colour) as a parameter, it will not be scaled, allowing you to control the appearance of the plot, as described in Section \@ref(setting-mapping).
...
-   **position**: The method used to adjust overlapping objects, like jittering, stacking or dodging.
    More details in Section \@ref(position).
...
You'll learn more about that in Section \@ref(scale-identity).

...
*   __mapping__: A set of aesthetic mappings, specified using the 
    ...
    [aesthetic mappings](#aes). If `NULL`, uses the default mapping
    set in `ggplot()`.
...
*   __data__: A dataset which overrides the default plot dataset. It is usually
    ...
    in more detail in [data](#data).
  
*   __geom__: The name of the geometric object to use to draw each observation. 
    Geoms are discussed in more detail in [geom](#data), and 
    ...
    as described in [setting vs. mapping](#setting-mapping). You can
...
*   __stat__: The name of the statistical tranformation to use. A statistical
    ...
    Learn more in [statistical transformations](#stat).
...
*   __position__: The method used to adjust overlapping objects, like
    jittering, stacking or dodging. More details in [position](#position).
...
* It enforces a clean separation of concerns: ggplot2 turns data frames into
  ...
  (learn more about that in [model visualisation](#modelvis)).
...
This is most useful if you always have a column that already contains colours. You'll learn more about that in [the identity scale](#scale-identity).
...
The result of this plot is rather surprising: low quality diamonds seem to be more expensive on average. We'll come back to this result in [removing trend](#trend).

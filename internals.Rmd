...
The code in our simplified print method reveals five distinct steps:

- First, it calls `set_last_plot()` to store the plot object internally. This is done to allow the `last_plot()` function to work, but otherwise does not affect the plotting process. 

- Second, it calls `ggplot_build()` where the data for each layer is prepared and organised into a standardised format suitable for plotting.

- Third, the prepared data is passed to the `ggplot_build()` and turns it into it into graphic elements stored in a gtable (we'll come back to what that is later). 

- Fourth, the gtable object is converted to an image with the assistance of the grid package.

- Fifth, the original ggplot object is invisibly returned to the user.
...
```{r echo=FALSE}
# this function mimics ggplot_build, but instead of returning the 
# ggplot_built object, it returns a list of objects that include
# intermediate states of interest for the exposition
...
The second step in the process is to map the position aesthetics using the position scales, which unfolds differently depending on the kind of scale involved. For continuous scales, the out of bounds function specified in the `oob` argument (Section \@ref(limits)) is applied at this point, and `NA` values in the layer data are removed. For discrete positions the change is more radical, because the values are matched to the `limits` values or the `breaks` specification provided by the user, and then converted to integer-valued positions. Finally, for binned position scales the continuous data is first cut into bins using the `breaks` argument, and the position for each bin is set to the midpoint of its range. The reason for performing the mapping at this stage of the process is consistency: no matter what type of position scale is used, it will look continuous to the stat and geom computations. This is important because otherwise computations such as dodging and jitter would fail for discrete scales. The original plot `p` has no position adjustment and both scales are continuous so the layer data remain unchanged:


```{r, echo=FALSE}
ggbuild(p)$positioned[[1]] %>% head(n = 3)
```

However, if we were plotting `p + xlim(2, 8)` the `oob` function -- `scales::censor()`  in this case -- would be applied at this point and `x` values below 2 would be replaced with `NA` as illustrated below:
...
As an example consider the histogram of engine displacements specified by `ggplot(mpg) + geom_histogram(aes(displ))`. Before the stat computations have been performed the data for the histogram layer simply contain the `x` co-ordinates and the required `PANEL` and `group` columns.

```{r echo=FALSE, message=FALSE}
bb <- ggbuild(ggplot(mpg) + geom_histogram(aes(displ)))
bb$positioned[[1]] %>% head(n = 3)
```

After the stat computations have taken place, the layer data are changed considerably:

```{r echo=FALSE}
bb$poststat[[1]] %>% head(n = 3)
```


At this point the geom takes over from the stat (almost). The first action it takes is to inspect the data, update its parameters and possibly make a first pass modification of the data (same setup as for stat). This is possibly where some of the columns gets reparameterised e.g. `x`+`width` gets changed to `xmin`+`xmax`. After this the position adjustment gets applied, so that e.g. overlapping bars are stacked, etc. To illustrate, consider a version of `p` that specified `stat = "jitter"` (or simply used `geom_jitter()`). Before this step in the transformation, the layer data are identical:

```{r, echo=FALSE}
ggbuild(ggplot(mpg) + geom_jitter(aes(displ, hwy, color = drv)))$poststat[[1]] %>% head(n = 3)

cat("\n")

ggbuild(p)$poststat[[1]] %>% head(n = 3)
```

But after this step, the two plots diverge. The first output below shows the first three rows from the layer data for `p` at this point in the process, and the second output shows the same rows for the jittered version of the plot:
...
ggbuild(ggplot(mpg) + geom_jitter(aes(displ, hwy, colour = drv)))$geompos[[1]] %>% head(n = 3)
```

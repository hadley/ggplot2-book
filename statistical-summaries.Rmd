...
These geoms assume that you are interested in the distribution of y conditional on x and use the aesthetics `ymin` and `ymax` to determine the range of the y values. If you want the opposite, see [coord_flip](#coord-flip). \index{Error bars} \indexf{geom\_ribbon} \indexf{geom\_smooth} \indexf{geom\_errorbar} \indexf{geom\_linerange} \indexf{geom\_crossbar} \indexf{geom\_pointrange}
...
*   Estimate the 2d density with `stat_density2d()`, and then display using one
    of the techniques for showing 3d surfaces in [surfaces](#surface).
    
*   If you are interested in the conditional distribution of y given x, then
    the techniques of [displaying distributions](#distribution) will also 
    be useful.

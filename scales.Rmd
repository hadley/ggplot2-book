...
### Scale names and types

There are a large number of user-facing scale functions in ggplot2, all of which follow a common naming scheme. You’ve probably already figured out the naming scheme for scales, but to be concrete, it’s made up of three pieces separated by "_":

1. `scale`
1. The name of the aesthetic (e.g., `colour`, `shape` or `x`)
1. The name of the scale (e.g., `continuous`, `discrete`, `brewer`).

From the user side, these are the only functions you should need to call. It may be useful, however, to recognise that as a practical matter ggplot2 classifies the scale functions into three fundamental scale types: continuous, discrete and binned. Accordingly, every scale in ggplot2 is constructed using one of three functions, `continuous_scale()`, `discrete_scale()` and `binned_scale()`. This is why, in Section \@ref(scale-position) where I discuss scales associated with position aesthetics, the text is subdivided into these sections. A similar structure applies for aesthetics based on colour \@ref(scale-colour), and other sections.
...
The most common continuous position scales are `scale_x_continuous()` and `scale_y_continuous()`, discussed earlier.

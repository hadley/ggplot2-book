...
1. You want to expand the limits to make multiple plots to match up.

It's most natural to think about the limits of position scales: they map directly to the ranges of the axes. But limits also apply to scales that have legends, like colour, size, and shape, which is particularly important if you want colours to be consistent across multiple plots. To modify the limits of a scale, specify the `limits` parameter of the scale:
...
ggplot(mpg, aes(x = hwy, y = ..count..)) + geom_histogram()
```

In the first version the y aesthetic is not directly specified by the user but is instead supplied by the geom (or more precisely, the stat). In the second version this is made explicit, and the y aesthetic is mapped to the computed variable `..count..`. Position scales in ggplot2 can be used to represent continuous variables and discrete ones. I discuss continuous position scales first.

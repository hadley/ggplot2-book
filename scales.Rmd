...
Discrete position scales are secretly continuous scales, with categories mapped to integer values. Illustrate by annotating a plot. Continuous data can be converted to discrete scales by binning. Illustrate this too.

```{r}
p <- ggplot(mpg, aes(x = hwy, y = class)) + geom_count()
p + annotate("text", x = 5, y = 1:7, label = 1:7) 
p + scale_x_binned(n.breaks = 10)
```

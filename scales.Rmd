...
Setting limits is pertinent to every scale family and works similarly across families. The limits, or domain, of a scale are usually derived from the range of the data. \index{Axis!limits} \index{Scales!limits} There are two reasons you might want to specify limits rather than relying on the data:
...
<!-- DN: I am very tempted to move this discussion to the guides chapter as it seems to be mostly about the legend. It seems to me that the better thing to do in this chapter is to briefly mention that scale_x_date() is an example of a continuous position scale and link the reader to the next chapter. -->

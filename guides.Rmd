...
`r columns(2, 1 / 2)`
```{r markdown-with-ggtext, cache.vars=toy}
...
`r columns(2, 2/3)`
```{r guide-names-helper, cache.vars=toy}
...
```{r guide-names-remove, cache.vars=toy}
...
`r columns(3, 2/3)`
```{r breaks-labels, cache.vars=toy}
...
```{r, cache.vars=toy}
leg <- ggplot(toy, aes(big, const, fill = big)) + 
  geom_tile() + 
  labs(x = NULL, y = NULL)
...
`r columns(3)`
```{r breaks-functions, cache.vars=toy}
...
`r columns(3)`
```{r legend-geom, echo = FALSE, cache.vars=toy}
...
`r columns(2, 2/3)`
```{r legend-position, cache.vars=toy}
base <- ggplot(toy, aes(up, up)) + 
...
`r columns(3, 1.5)`
```{r legend-position-man, cache.vars=toy}
base <- ggplot(toy, aes(up, up)) + 
  geom_point(aes(colour = txt), size = 3)

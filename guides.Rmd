...
You can override the default guide using the `guide` argument of the corresponding scale function, or more conveniently, the `guides()` helper function. The `guides()` helper works in a similar way to the `labs()` helper function: both take the name of different aesthetics (e.g., color, x, fill) as arguments, and you can then override the default guide or label by assigning a value to those arguments. The three plots below illustrate how the `guides()` function works:

...
<!-- HW: I reorganised this a bit; probably needs proof reading to make sure I didn't mess anything up -->
...
The early sections of this chapter highlight functionality that is shared by axes and legends. Section \@ref(scale-name) discusses the `name` argument, while Section \@ref(breaks-labels) covers the `breaks` and `labels` arguments in more detail. 
...
Tater parts of the chapter focuses mostly on legends because they are more complicated than axes:
...
In ggplot2 the axis or legend title is specified by `name`, which is always the first argument to the scale function. Usually this argument takes a text string as input, using `\n` to specify line breaks, but you can supply mathematical expressions wrapped `quote()`, as described in `?plotmath`
...
<!-- HW: in the last example breaks overlap a lot. Not sure what do. Options:
* reduce n instead of increasing it
* drop the first plot since it's the same as the second
* give a sneak preview of angle = 90 or similar guide_axis() option
-->
...
<!--HW: I use date + date-time then date/time to mean date, date-time, or time -->
...
<!-- HW: I moved this to the end of the section -->
...
<!-- HW: I removed the example here because I don't love these functions. They always feel a bit clunkly to me -->
...
<!--HW: removed the caution since ggplot2 now automatically warns if the the lengths don't match -->
...
<!-- HW: random comment I think either you or past-me uses ggplot2 to a bit too often - I think it's usually obvious that we're talking about ggplot2 :) -->
...
<!-- HW: Instead of "as the example below illustrates" or similar, I usually just stick with a colon -->
...
The guide functions have numerous examples in the documentation that illustrate all of their arguments. Many of the arguments to the guide function are equivalent to theme settings (Chapter \@ref(polishing)) likes text colour, size, font etc, but only apply to a single guide. Here I'll focus on the non-themeing arguments.
...
<!-- HW: I prefer `foo()` to "the `foo()` function" -->

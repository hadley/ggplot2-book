...
As a consequence there are some extra options that only apply to legends, and the later sections in the chapter focus on this legend-specific behaviour. Note that some plots in this chapter use real data (usually `mpg`) but for simplicity of exposition I will sometimes use this `toy` data frame:
...
One of the most common tasks when creating plots is customising the title of the plot axes/legends. This can be achieved by setting the first argument to the scale function, which specifies the scale `name`. You can supply text strings (using `\n` for line breaks) or mathematical expressions in `quote()` (as described in `?plotmath`):  \index{Axis!title} \index{Legend!title}
...
Note that it is also possible to use (a limited subset of) markdown in axis labels with the help of the ggtext package [@ggtext], by setting the relevant theme element to `ggtext::element_markdown()`. Modifying theme settings is discussed further in Chapter \@ref(polishing).

`r columns(2, 1)`
```{r markdown-with-ggtext, cache.vars=toy}
base <- ggplot(toy, aes(const, up)) + 
  geom_point() + 
  xlab("Use *italics* in label") +
  ylab("Use **boldface** in label")

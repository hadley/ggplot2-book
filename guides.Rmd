...
The code below illustrates the same ideas applied to legends:
...
Note that if you set the `labels` argument (for axes or legends) you must also set `breaks`. Otherwise, if the data changes, the breaks will no longer align with the labels. 
...
base
base + scale_y_discrete(labels = c(a = "ant", b = "bat", c = "cat", d = "dog"))
```
...
A variation on this is set `labels = NULL`, which removes the labels but leaves the other properties of the axis/legend unchanged:

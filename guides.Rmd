...
Because tweaking axis and legend labels is such a common task, there are three 
helper functions in ggplot2 that can save you some typing. The first two are `xlab()` and `ylab()`, each of which takes a single argument that specifies the corresponding axis label. The `labs()` helper function is more flexible and can be used to set the name for one or more scale, using name-value pairs such as `x = "X axis"` or `fill = "fill legend"`. All three are illustrated below:
...
In the same way that the `name` argument to a scale function governs axis titles and legend titles, the `breaks` argument is used to control which values appear as tick marks on axes and as keys on legends. Every break is associated with a label and these can be changed by setting the `labels` argument to the scale function. If you do set the `labels` argument, be sure to set `breaks` as well: if you don't and the data changes, the breaks will no longer align with the labels. The code below shows some basic examples for axes:
 \index{Axis!ticks} \index{Axis!breaks} \index{Axis!labels} \index{Legend!keys}
...
In the simple examples above I specified the vector of values for `breaks` and `labels` manually, but ggplot2 also allows you to pass functions either of these arguments. The function passed to `breaks` should have one argument that specifies the limits of the scale (a numeric vector of length two), and it should return a numeric vector of breaks. A function passed to `labels` should accept a numeric vector of breaks as input and return a character vector of labels (the same length as the input). You can write your own break and labelling functions, but in many cases there is no need, thanks to the scales package [@scales].

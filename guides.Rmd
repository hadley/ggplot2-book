...
Notice the difference betweeen setting an offset of 200 and -200.

Other break functions are useful in different situations. For example an economical way to produce nicely spaced breaks similar to the log-scale example in Section \@ref(minor-breaks) is shown below:

`r columns(1, 1/3, 1)`
```{r}
base + scale_x_log10(breaks = scales::breaks_log(n = 10))
```

The same approach can be applied to minor breaks:

`r columns(1, 1/3, 1)`
```{r}
base + scale_x_log10(
  breaks = scales::breaks_log(n = 10), 
  minor_breaks = scales::breaks_log(n = 30)
)
```

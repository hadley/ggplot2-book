...
A major requirement of a good data analysis is flexibility. If the data changes, or you discover something that makes you rethink your basic assumptions, you need to be able to easily change many plots at once. The main inhibitor of flexibility is code duplication. If you have the same plotting statement repeated over and over again, you have to make the same change in many different places. Often just the thought of making all those changes is exhausting!
...
You're not just limited to layers, you can also put any of the following object in that list:
...
* An `aes()` object, which will combined with the existing default
  aesthetic mapping.
  
* Scales, which override existing scales (with a warning if they're already
  been set by the user)

* Coordinate systems, which overrides the existing coordinate system.

* Facetting specification, which overrides the existing facetting
  specification.
...
1.  To make the best use of space, many examples in this book turn off 
    ...
    into multiple place, but it would make more sense to create a reusable 
...
You can take a similar approach to drawing parallel coordinates plots (PCPs) with ggplots. PCPs require a transformation of the data, so you can write two functions: one that does the transformation and one that generates the plot. I strongly recommend keeping the transformation and plotting separate because it makes life much easier if you later want to reuse the same transformation for a different visualisation.
...
Note that this is only a problem with `mapping`, not with any other arguments. All other arguments are evaluated immediately so their values (not a reference to a name) are stored in the plot object.

...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}
    \DataTypeTok{lcarat =} \KeywordTok{log}\NormalTok{(carat),}
    \DataTypeTok{lprice =} \KeywordTok{log}\NormalTok{(price)}
  \NormalTok{)}
\NormalTok{diamonds2}
...
\StringTok{  }\KeywordTok{geom_smooth}\NormalTok{(}\DataTypeTok{method =} \StringTok{"lm"}\NormalTok{, }\DataTypeTok{se =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{size =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> (Intercept)     8.46    0.00146    5789        0}
\CommentTok{#> lcarat          1.70    0.00208     816        0}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{ggplot}\NormalTok{(diamonds2, }\KeywordTok{aes}\NormalTok{(carat, rel_price)) +}\StringTok{ }\KeywordTok{geom_bin2d}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{xgrid <-}\StringTok{ }\KeywordTok{seq}\NormalTok{(-}\FloatTok{1.5}\NormalTok{, }\FloatTok{1.5}\NormalTok{, }\DataTypeTok{length =} \DecValTok{11}\NormalTok{)}
\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{logx =} \NormalTok{xgrid, }\DataTypeTok{x =} \KeywordTok{round}\NormalTok{(}\KeywordTok{exp}\NormalTok{(xgrid), }\DecValTok{2}\NormalTok{))}
\CommentTok{#>    logx    x}
\CommentTok{#> 1  -1.5 0.22}
\CommentTok{#> 2  -1.2 0.30}
\CommentTok{#> 3  -0.9 0.41}
\CommentTok{#> 4  -0.6 0.55}
\CommentTok{#> 5  -0.3 0.74}
\CommentTok{#> 6   0.0 1.00}
\CommentTok{#> 7   0.3 1.35}
\CommentTok{#> 8   0.6 1.82}
\CommentTok{#> 9   0.9 2.46}
\CommentTok{#> 10  1.2 3.32}
\CommentTok{#> 11  1.5 4.48}
\end{Highlighting}
\end{Shaded}

For example, a \texttt{rel\_price} of 0.6 means that it's 1.8x of the
expected price (i.e 80\% more expensive than expected); a relative price
of -0.3 means that it's 0.74x of the expected price (i.e.~26\% cheaper
than expected). \index{Log!transform}
...
\CommentTok{#> 1       D      Fair  3939   -0.0524}
\CommentTok{#> 2       D      Good  3309   -0.0327}
\CommentTok{#> 3       D Very Good  3368    0.0719}
\CommentTok{#> 4       D   Premium  3513    0.0758}
\CommentTok{#> 5       D     Ideal  2595    0.1506}
\CommentTok{#> 6       E      Fair  3516   -0.1192}
\CommentTok{#> ..    ...       ...   ...       ...}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> Warning: Removed 430 rows containing missing values (geom_path).}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> Warning: Removed 568 rows containing non-finite values}
\CommentTok{#> (stat_summary).}
\CommentTok{#> Warning: Removed 430 rows containing missing values (geom_path).}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{do}\NormalTok{(}\DataTypeTok{mod =} \KeywordTok{lm}\NormalTok{(}
    \KeywordTok{log}\NormalTok{(sales) ~}\StringTok{ }\KeywordTok{factor}\NormalTok{(month), }
...
\CommentTok{#> 1    Abilene     0.530         0.500 0.195      17.9 1.50e-23    12}
\CommentTok{#> 2   Amarillo     0.449         0.415 0.209      13.0 7.41e-18    12}
\CommentTok{#> 3  Arlington     0.513         0.483 0.185      16.8 2.75e-22    12}
\CommentTok{#> 4     Austin     0.487         0.455 0.215      15.1 2.04e-20    12}
\CommentTok{#> 5   Bay Area     0.555         0.527 0.183      19.9 1.45e-25    12}
\CommentTok{#> 6   Beaumont     0.430         0.395 0.191      12.0 1.18e-16    12}
\CommentTok{#> ..       ...       ...           ...   ...       ...      ...   ...}
...
either summarises complexity (e.g. \texttt{df}) or fit (e.g.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{top3 <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Midland"}\NormalTok{, }\StringTok{"Irving"}\NormalTok{, }\StringTok{"Denton County"}\NormalTok{)}
\NormalTok{bottom3 <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Brownsville"}\NormalTok{, }\StringTok{"Harlingen"}\NormalTok{, }\StringTok{"McAllen"}\NormalTok{)}
...
\CommentTok{#> 1  Abilene    (Intercept)    4.535    0.0488     92.88 7.90e-151}
\CommentTok{#> 2  Abilene factor(month)2    0.245    0.0690      3.55  4.91e-04}
\CommentTok{#> 3  Abilene factor(month)3    0.468    0.0690      6.77  1.83e-10}
\CommentTok{#> 4  Abilene factor(month)4    0.519    0.0690      7.52  2.76e-12}
\CommentTok{#> 5  Abilene factor(month)5    0.635    0.0690      9.20  1.06e-16}
\CommentTok{#> 6  Abilene factor(month)6    0.695    0.0690     10.06  4.37e-19}
\CommentTok{#> ..     ...            ...      ...       ...       ...       ...}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> 1  Abilene     2    0.245     0.069      3.55 4.91e-04}
\CommentTok{#> 2  Abilene     3    0.468     0.069      6.77 1.83e-10}
\CommentTok{#> 3  Abilene     4    0.519     0.069      7.52 2.76e-12}
\CommentTok{#> 4  Abilene     5    0.635     0.069      9.20 1.06e-16}
\CommentTok{#> 5  Abilene     6    0.695     0.069     10.06 4.37e-19}
\CommentTok{#> 6  Abilene     7    0.661     0.069      9.58 9.81e-18}
\CommentTok{#> ..     ...   ...      ...       ...       ...      ...}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ggplot}\NormalTok{(months, }\KeywordTok{aes}\NormalTok{(month, }\KeywordTok{exp}\NormalTok{(estimate))) +}
...
\KeywordTok{ggplot}\NormalTok{(coef_sum, }\KeywordTok{aes}\NormalTok{(max, }\KeywordTok{reorder}\NormalTok{(city, max))) +}\StringTok{ }
\StringTok{  }\KeywordTok{geom_point}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
details): \indexf{augment}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{obs_sum <-}\StringTok{ }\NormalTok{models %>%}\StringTok{ }\KeywordTok{augment}\NormalTok{(mod)}
\NormalTok{obs_sum}
\CommentTok{#> Source: local data frame [8,034 x 13]}
\CommentTok{#> Groups: city [46]}
\CommentTok{#> }
\CommentTok{#>       city log.sales. factor.month. .fitted .se.fit .resid   .hat}
\CommentTok{#>      (chr)      (dbl)        (fctr)   (dbl)   (dbl)  (dbl)  (dbl)}
\CommentTok{#> 1  Abilene       4.28             1    4.53  0.0488 -0.258 0.0625}
\CommentTok{#> 2  Abilene       4.58             2    4.78  0.0488 -0.195 0.0625}
\CommentTok{#> 3  Abilene       4.87             3    5.00  0.0488 -0.135 0.0625}
\CommentTok{#> 4  Abilene       4.58             4    5.05  0.0488 -0.469 0.0625}
\CommentTok{#> 5  Abilene       4.95             5    5.17  0.0488 -0.221 0.0625}
\CommentTok{#> 6  Abilene       5.05             6    5.23  0.0488 -0.180 0.0625}
\CommentTok{#> ..     ...        ...           ...     ...     ...    ...    ...}
\CommentTok{#> Variables not shown: .sigma (dbl), .cooksd (dbl), .std.resid (dbl),}
\CommentTok{#>   .rownames (chr), log(sales) (dbl), factor(month) (fctr)}
\end{Highlighting}
\end{Shaded}

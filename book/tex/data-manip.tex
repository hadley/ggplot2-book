...
solve these problems (and more!) with the dplyr package.
...
frame. The second and subsequent arguments control the details of the
transformation, and are always interpreted in the context of the data
frame so you can refer to variables directly. I'll also explain each in
the same way: I'll show you a motivating examples using the
\texttt{diamonds} data, give you more details about how the function
works, and finish up with some exercises for you to practice your skills
with.
...
analysis strategy is to start with just one obseravation unit (one
...
it's useful to think about partioning the data into the common and the
...
\texttt{filter()} works with exclusuively with observations and
...
we're just going to throw them out and focus on what remains. To save a
little typing, we can take advantage of the fact that multiple arguments
to \texttt{filter()} are combined with and.
...
line to make it clear that for most diamands, \texttt{x} and \texttt{y}
...
\item
  \texttt{x \%in\% c("a", "b", "c")}: \texttt{x} is one of the values in
  the right hand side.
\item
...
\item
  ...
  are \texttt{TRUE}, but not both (e\_\_x\_\_clusive or).
\end{itemize}
...
\texttt{x \textgreater{} 10 \textbar{} is.na(x)}. In other parts of R,
you'll sometimes need to convert missing values into \texttt{FALSE}. You
can do that with \texttt{x \textgreater{} 10 \& !is.na(x)}
...
\item
  Install ggplot2movies and look at the movies that have a missing
  budget. How are they different from the movies with a budget? (Hint:
  try a frequency polygon plus \texttt{colour = is.na(budget)}.)
\item
  What is \texttt{NA \& FALSE} and \texttt{NA \textbar{} TRUE}? Why? Why
  doesn't \texttt{NA * 0} equal zero? What number times zero does not
  equal 0? What do you expect \texttt{NA \^{} 0} to equal? Why?
\end{enumerate}
...
\begin{itemize}
\item
  Log-transformations are often useful. They turn multiplicate
...
\item
  ...
  \texttt{x / y} takes values (1, Inf). See T{ö}rnqvist, Vartia, and
...
hand: that gives you more flexibility and and you can use the summaries
for other purposes. \indexf{group\_by} \indexf{summarise}
...
clarities have lower prices? We'll see why this is the case and what to
do about it in \hyperref[sub:trend]{removing trend}.
...
by clarity to also include the number of obseravtions in each group, and
...
number of hits divded by the number of at bats. This is the proportion
that a batter hits the ball. Who's the best batter according to this
metric?
...
\NormalTok{ba <-}\StringTok{ }\KeywordTok{summarise}\NormalTok{(per_player, }\DataTypeTok{ba =} \KeywordTok{sum}\NormalTok{(H, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{) /}\StringTok{ }\KeywordTok{sum}\NormalTok{(AB, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{))}
...
\NormalTok{ba <-}\StringTok{ }\KeywordTok{summarise}\NormalTok{(per_player, }
  ...
  \DataTypeTok{ab =} \KeywordTok{sum}\NormalTok{(AB)}
\NormalTok{)}
...
\item
  Compute a histogram of carat by ``hand''. Use a binwidth of 0.1.
...
righ hand side (RHS). Each of these pairs of calls is equivalent:
...
\begin{itemize}
\item
  \texttt{arrange()}, that can be useful when you're looking at the data
  from the console. It doesn't affect visualisations because ggplot2
  doesn't care about the order of the rows.
\item
  \texttt{select()} picks variables based on their names. Useful when
  you have very many variables and want to focus on just a few for
  analysis.
\item
...
\item
  ...
  to the database. See \texttt{vignette("databases", package = "dplyr")}
  for the details.
\end{itemize}
...
T{ö}rnqvist, Leo, Pentti Vartia, and Yrj{ö} O Vartia. 1985. ``How Should

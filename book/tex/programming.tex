...
\NormalTok{\}}
\KeywordTok{ggplot}\NormalTok{(mpg, }\KeywordTok{aes}\NormalTok{(class, cty)) +}\StringTok{ }\KeywordTok{geom_mean}\NormalTok{()}
\KeywordTok{ggplot}\NormalTok{(mpg, }\KeywordTok{aes}\NormalTok{(class, cty)) +}\StringTok{ }\KeywordTok{geom_mean}\NormalTok{(}\DataTypeTok{se =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  in the legend. \indexc{show.legend}
\end{itemize}
...
To get your start, here's one approach using \texttt{modifyList()} and

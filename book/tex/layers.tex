...
be ommitted, in which case they correspond to the x and y variables.
...
\texttt{colour = "red"}). \index{Aesthetics!setting}
...
\item
  ...
  \item
    ...
    \itemsep1pt\parskip0pt\parsep0pt
    \item
      \texttt{geom\_bar(stat = "identity")}: a bar chart of precomputed
      summaries.
    \item
...
for your data will usually make it much easier to draw a plot you want.
...
lists the variables that it creates in its documentation. \indexc{..}
...
\StringTok{  }\KeywordTok{geom_freqpoly}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{group =} \NormalTok{cut)) +}\StringTok{ }

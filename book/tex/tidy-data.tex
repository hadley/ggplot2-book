...
(If it looks familiar it's because it's dervied from the
...
\texttt{x} and \texttt{y} variables). I call the second form is called
...
Note that the columns have names that are not standard varible names in
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{economics <-}\StringTok{ }\KeywordTok{gather}\NormalTok{(ec2, year, rate, }\StringTok{`}\DataTypeTok{2006}\StringTok{`}\NormalTok{:}\StringTok{`}\DataTypeTok{2015}\StringTok{`}\NormalTok{, }
...
because it represents dates that haven't occured yet.)
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ggplot}\NormalTok{(economics, }\KeywordTok{aes}\NormalTok{(year +}\StringTok{ }\NormalTok{(month -}\StringTok{ }\DecValTok{1}\NormalTok{) /}\StringTok{ }\DecValTok{12}\NormalTok{, rate)) +}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{()}

\KeywordTok{ggplot}\NormalTok{(economics, }\KeywordTok{aes}\NormalTok{(month, rate, }\DataTypeTok{group =} \NormalTok{year)) +}
...
\CommentTok{#>    (chr) (int) (int) (fctr) (dbl)}
\CommentTok{#> 1   Anne    35 16156  week1   100}
\CommentTok{#> 2    Ben    41 16138  week1   110}
\CommentTok{#> 3   Carl    33 16162  week1   125}
\CommentTok{#> 4   Anne    35 16156  week2   100}
\CommentTok{#> 5    Ben    41 16138  week2   100}
...
\CommentTok{#> Warning: Too few values at 7 locations: 1, 2, 3, 4, 5, 7, 8}
\NormalTok{bpd_2}
\CommentTok{#> Source: local data frame [9 x 6]}
\CommentTok{#> }
\CommentTok{#>     name   age start   week   sys   dia}
\CommentTok{#>    (chr) (int) (int) (fctr) (chr) (chr)}
\CommentTok{#> 1   Anne    35 16156  week1   100    NA}
\CommentTok{#> 2    Ben    41 16138  week1   110    NA}
\CommentTok{#> 3   Carl    33 16162  week1   125    NA}
\CommentTok{#> 4   Anne    35 16156  week2   100    NA}
\CommentTok{#> 5    Ben    41 16138  week2   100    NA}
...
use arrange (which you'll learn about in the next chapter) to order the
rows to keep the records for each person together.
...
\NormalTok{bpd_4 <-}\StringTok{ }\KeywordTok{arrange}\NormalTok{(bpd_3, name, start)}
\NormalTok{bpd_4}
\CommentTok{#> Source: local data frame [9 x 6]}
\CommentTok{#> }
\CommentTok{#>     name   age start  week   sys   dia}
\CommentTok{#>    (chr) (int) (int) (int) (chr) (chr)}
\CommentTok{#> 1   Anne    35 16156     1   100    NA}
\CommentTok{#> 2   Anne    35 16156     2   100    NA}
\CommentTok{#> 3   Anne    35 16156     3   120    NA}
\CommentTok{#> 4    Ben    41 16138     1   110    NA}
\CommentTok{#> 5    Ben    41 16138     2   100    NA}
\CommentTok{#> 6    Ben    41 16138     3   135    NA}
\CommentTok{#> ..   ...   ...   ...   ...   ...   ...}
\end{Highlighting}
\end{Shaded}
...
Imagine you're intersted in the effect of an intervention on test
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{mutate}\NormalTok{(scores_2, }\DataTypeTok{diff =} \NormalTok{post -}\StringTok{ }\NormalTok{pre)}

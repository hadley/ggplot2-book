...
```{r}
ggplot(economics_long, aes(date, value01, linetype = variable)) + geom_line()
```

With five categories the plot is quite difficult to read, and it is unlikely you will want to use the linetype aesthetic for more than that. The default "palette" for linetype is supplied by the `scales::linetype_pal()` function, and includes the 13 separate linetypes shown below:

`r columns(1, 1.2)`
```{r}
linetypes <- scales::linetype_pal()
df <- data.frame(
  value = letters[1:13],
  type = linetypes(13)
)
ggplot(df, aes(linetype = value)) +
  geom_segment(aes(x = 0, xend = 1, y = value, yend = value), show.legend = FALSE) +
  geom_text(aes(x = 1.1, y = value, label = type), hjust = 0) + 
  theme(panel.grid = element_blank()) +
  scale_x_continuous(NULL, NULL, limits = c(0, 1.4)) 
```

The data used to specify a line type in ggplot2 follow the same rules as in base graphics. Valid line types can be set using a human readable character string: `"blank"`, `"solid"`, `"dashed"`, `"dotted"`, `"dotdash"`, `"longdash"`, and `"twodash"` are all understood. Alternatively, you can specify a line type as an integer between 0 and 6, where 0 produces a blank line, 1 is solid, and so on. However, as with base graphics you can also exercise finer grained control over the line type by specifying a string with up to 8 hexadecimal values (i.e., from 0 to F). In this specification, the first value is the length of the first line segment, the second value is the length of the first space between segments, and so on. This allows you to specify your own palette function for the linetype:

`r columns(1, 2/3)`
```{r}
linetypes <- function(n) {
  types <- c("53",  "75", "97", "111115", "11111117")
...
One example of a manual colour scale appears in Section \@ref(manual-colour). In this example I'll show a creative use of `scale_colour_manual()` to display multiple variables on the same plot and show a useful legend. In most plotting systems, you'd colour the lines and then add a legend: \index{Data!longitudinal}

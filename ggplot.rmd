...
You might wonder when to use faceting, and when to use aesthetics. You'll learn more about the relative advantages and disadvantages of each in [grouping vs. faceting](#sub:group-vs-facet).
...
*   `method = "gam"` fits a generalised additive model provided by the __mgcv__ 
    ...
    \index{mgcv} \index{Model!generalised additive}
...
To display this sort of data, you need to tell `geom_bar()` to not run the default statistical transformation which bins and counts the data. However, for this sort of data, I prefer `geom_point()` because points take up less space than bars, and don't require that the y axis includes 0. 

...
* How to display additional categorical variables in a plot using
  small multiples created by faceting, [faceting](#sec:qplot-faceting).
...
Almost every plot maps a variable to `x` and `y`, so naming these aesthetics is tedious, and the first two unnamed arguments to `aes()` will be mapped to `x` and `y`. This means the following code is identical to the example above:

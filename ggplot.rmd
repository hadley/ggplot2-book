...
If you want to set an aesthetic to a fixed value, without scaling it, do so in the individual geom, and outside of `aes()`. Compare the following two plots:
...
But if you have this sort of data, you should seriously consider using `geom_point()` instead: they take up less space, and don't require that the y axis includes 0.
...
qplot(displ, hwy, data = mpg, colour = I("red"))
```

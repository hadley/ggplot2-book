...
We can fix the first problem by logging sales:
...
We can fix the second problem using the same basic technique as the previous section. We'll fit a linear model and look at the residuals. This time we'll use a categorical predictor to remove the month effect. First we check that the technique works by applying it to a single city. It's always a good idea to start simple so that if something goes wrong you can more easily pin-point the problem.

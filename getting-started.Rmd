...
-   The three key components of every plot: data, aesthetics and geoms, Section \@ref(basic-use).

-   How to add additional variables to a plot with aesthetics, Section \@ref(aesthetics).

-   How to display additional categorical variables in a plot using small multiples created by faceting, Section \@ref(qplot-faceting).

-   A variety of different geoms that you can use to create different types of plots, Section \@ref(plot-geoms).

-   How to modify the axes, Section \@ref(axes).

-   Things you can do with a plot object other than display it, like save it to disk, Section \@ref(output).
...
You can access the data by loading ggplot2: \index{Data!mpg@\texttt{mpg}}
...
You'll learn how to override them in Chapter \@ref(scale-colour).
...
This is an important technique and you'll learn more about it in Section \@ref(setting-mapping).
...
You'll learn more about faceting in Section \@ref(facet), but it's such a useful technique that you need to know it right away.
\index{Faceting}
...
You'll learn more about the relative advantages and disadvantages of each in Section \@ref(group-vs-facet).
...
You'll learn more in Chapters \@ref(individual-geoms) and \@ref(collective-geoms).
...
We could draw a scatterplot of unemployment rate vs. length of unemployment, but then we could no longer see the evolution over time.
...
This is explained in more depth in Chapter \@ref(collective-geoms).
...
-   Render it on screen with `print()`.
    This happens automatically when running interactively, but inside a loop or function, you'll need to `print()` it yourself.
    \indexf{print}

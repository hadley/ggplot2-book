...
Since an input variable is either discrete or continuous, the domain is either a set of values (stored as a factor, character vector, or logical vector) or an interval on the real line (stored as a numeric vector of length 2). For example, in the mammals sleep dataset, the domain of the discrete variable \var{vore} is \{carni, herbi, omni\}, and the domain of the continuous variable \var{bodywt} is $[0.005, 6654]$.  We often think of these as data ranges, but here we are focussing on their nature as input to to the scale, i.e.\ as a domain of a function.
...
\input{_include/caa21255ab2e02424cc7dd23f609ab7d.tex}
% END
...
\begin{itemize}
  ...
  \input{_include/f40256b38d14103e0297781a38749216.tex}  
  % END
  ...
  \input{_include/8b0f5035f4a245711b597770cb500241.tex}  
  % END
...
\input{_include/60b00f78d2957804cdb080fa8eed448d.tex}
% END
...
Date and times are basically continuous values, but with have special ways of labelling the axes. Currently, only dates of class \code{date} and times of class \code{POSIXct} are supported. If your dates are in a different format you will need to convert them with \f{as.Date} or \f{as.POSIXct}.
...
\begin{itemize}
  ...
  \item The \code{format} argument specifies how the tick labels should be formatted. Table~\ref{tbl:dates} lists the special characters used to display components of a date.  For example, if you wanted to display dates of the form 14/10/1979, you would use the string \verb|"%d/%m/%y"|.
\end{itemize}
...
%   apperance, (middle) breaks every 10 years, and (right) scale restricted
...
%   format = "%Y-%m-%d"
% )
\input{_include/d339764eaeefc715cba4fadef7c7cb38.tex}
% END
...
\begin{itemize}
  ...
  \item \f{scale_colour_gradient2} and \f{scale_fill_gradient2}: a three--colour gradient, low--med--high.  As well as \code{low} and \code{high} colours, these scales also have a \code{mid} colour for the colour of the midpoint.  The midpoint defaults to 0, but can be set to any value with the \code{midpoint} argument.  This is particularly useful for creating diverging colour schemes.
...
Colour gradients are often used to show the height of a 2d surface.  In the following example we'll use the surface of a 2d density estimate of the \code{faithful} dataset \citep{azzalini:1990}, which records the waiting time between eruptions and during of each eruption for the Old Faithful geyser in Yellowstone Park. Figure~\ref{fig:gradient} shows three gradients applied to this data, created with the following code.  Note the use of limits: this parameter is common to all scales.
...
\input{_include/98dbaf663c1285064ac3d762242e2983.tex}
% END
...
\input{_include/e280839109a72d96107749732eb5f512.tex}
% END
...
\input{_include/0e4789ccb4bbb3fc183f6596e82fd7f3.tex}
% END
...
\input{_include/8caf406a171c8a9ed386390fa1aab641.tex}
% END
...
\input{_include/c4b84f00147344d820846a9f55b31ec9.tex}
% END
...
\input{_include/cc98006dfd517c2681eaba680caa77a3.tex}
% END
...
%    coord_equal()
\input{_include/30eac497c5638a6ce7796a852e75465e.tex}
% END
...
\input{_include/ab372333b199d1aa8ae6777fe6e4136a.tex}
% END
...
\input{_include/ca8dd5324d1889a77746520612bbc278.tex}
% END

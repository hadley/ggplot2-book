...
\begin{itemize}
  ...
  \item Designed to make use of colour
...
ggplot is designed to work in a layered fashion, so that the raw data is one layer and statistical summaries are a layer and so on.  It allows an analyst to produce graphics using the same structured thinking that s/he uses to design an analysis.  In that way, it's especially helpful for students who have not yet developed the structured approach to analysis used by experts.
...
there was no underlying theory to existing graphics software - just a big collection of special cases  yeah, that's it. just a list -- one of these, and one of those, and one of the other. 
...
% Chapter Seven introduces my philosophy of data.  This chapter isn't crucial, but will help you understand the type of data \ggplot expects, and how to transform your data into that format.  It also discusses faceting in more detail, and discusses some ideas for combining modelling and graphics.
...
Two appendices provide additional useful information.  Appendix~\ref{cha:specifications} describes how colours, shapes, line types and sizes can be specified by hand, and Appendix~\ref{cha:translating} shows how to translate from base graphics, lattice graphics, and Wilkinson's {\sc gpl} to \ggplot syntax.

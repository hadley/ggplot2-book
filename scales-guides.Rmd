...
In practice you would typically use `labs()` for this, discussed in Section \@ref(titles), but it is conceptually helpful to understand that axis labels and legend titles are both examples of scale names: see Section \@ref(scale-name).
...
<!-- DN: Use this section to highlight connection between labs() in annotations chapter and the names argument -->
...
Although the default behaviour is to convert the **o**ut **o**f **b**ounds values to `NA`, you can override this by setting `oob` argument of the scale, a function that is applied to all observations outside the scale limits. The default is `scales::oob_censor()` which replaces any value outside the limits with `NA`. Another option is `scales::oob_squish()` which squishes all values into the range. An example using a fill scale is shown below:
...
- colour bar in Section \@ref(guide-colourbar)
- colour steps in Section \@ref(guide-coloursteps)
- legend in Section \@ref(guide-legend)
- axis in Section \@ref(guide-axis)
- bins in Section \@ref(guide-bins)

<!-- DJN: show examples where something other than the default guide is used -->

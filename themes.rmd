...
* `theme_linedraw()`: A theme with only black lines of various widths on white 
  backgrounds, reminiscent of a line drawing.  \indexf{theme\linedraw}
...
* `theme_void()`: A completely empty theme.  \indexf{theme\void}
...
You're not limited to the themes built-in to ggplot2. Other packages, like ggtheme by Jeffrey Arnold, add even more. Here's a few of my favourite from ggtheme: \index{ggtheme}
...
Modifying an individual theme component looks like `plot + theme(element.name = element_function())`.  In this section you'll learn about the basic element functions, and then in the next section, you'll see all the elements that you can modify. \indexf{theme}
...
The legend elements control the apperance of all legends. You can also modify the appearance of individiual legends, by modifying the same elements in `guide_legend()` or `guide_colourbar()`.
...
When saving a plot to use in another program, you have two basic choices of  output: raster or vector: \index{Saving} \index{Exporting} \index{Publication!saving output}
...
Figure \ref{fig:vector-raster} illustrates the basic differences for with circle. A good description is available at <http://tinyurl.com/rstrvctr>. 
...
* You have a plot (e.g. a scatterplot) with thousands of graphical objects 
  (i.e. points). A vector version will large and slow to render.
...
* For raster graphics (i.e. `.png`), the `dpi` argument controls the resolution 
  ...
  you may want to use 600 for particularly high-resolution output, or 96 for 

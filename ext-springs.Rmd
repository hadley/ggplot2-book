...
<!-- HW: Maybe only introduce this problem later in "testing the geom"? Otherwise it's a long gap between tension and resolution -->
...
It does have some limitations still, because the units for `diameter` and `tension` are expressed relative to the scale of the raw data. The actual diameter of a spring with `diameter = 0.5` will be different depending on the axis limits, and if the x and y axes are not on the same scale the shape of the spring will be distorted. 
...
<!-- HW: Maybe introduce tabular grid in a later paragraph -->
...
<!-- HW: I found the high transparency hard to read -->
...
<!-- HW: do we need to mention data units here? I think maybe ggplot2 doesn't use them, but might be worth mentioning anyway --> 

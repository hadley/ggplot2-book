...
2.  The new stat only allows fixed diameter and tension, so we'll next allow these to be set as parameters  (Section \@ref(spring2)).
...
spring <- circle %>% 
  mutate(
    motion = seq(0, 1.5, length.out = 100),
    x = x + motion,
    type = "spring"
  )
...
In this case our spring has only looped around once, but if we were to continue tracing the circle while moving along the x axis we'd end up with a spring with multiple loops. The faster we move the "pen", the more we will stretch the spring. This gives us some insight into the two parameters that characterise our springs:

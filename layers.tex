...
\begin{itemize}
  \item Data, \secref{sec:data}.

  \item Aesthetic mappings, \secref{sec:aes}.  Including more information about how layer settings override the plot defaults, the difference between setting values and mapping aesthetics, and the important group aesthetic.  

  \item Geometric object, \secref{sec:geom} describes geoms in a little more detail and lists all of the available geoms. 

  \item Statistical transformation, \secref{sec:stat}.

  \item Position adjustment, \secref{sec:position}.
\end{itemize}

Section~\ref{sec:pull-together} concludes by introducing you to some plotting techniques that take advantage of what you have learned in this chapter.
...
The restriction on the data is simple: It must be a data frame.  Unlike other graphics packages in R, \ggplot provides very few ways to get your data in.  For example, lattice functions can take an optional data frame or use vectors directly from the global environment.  Base methods often work with vectors, data frames or other R objects.  \f{ggplot} only works with data in a data frame.  Section \ref{sec:methods} discusses this philosophy in more depth. While \f{qplot} does work with vectors in your workspace, it has to use some heuristics and it doesn't always get it right.  This means that is usually best to explicitly arrange your data in the format that \ggplot uses.  

This can make the plot specification more verbose, but you can be more sure about exactly what is going on with your data.  Your data is very important, and it's better to be explicit about exactly what is done with it.  It also allows a cleaner separation of concerns so that the \ggplot deals only with plotting data, not wrangling it into different forms, for which you might find the \code{plyr} or \code{reshape} packages helpful. 

The positive consequence of this decision is that its very easy to produce the same plot for different data sets.  If you want to change (or update) the data in a plot, you can replace the default dataset with \code{\%+\%}, as shown in the following example.  This is an easy way to experiment with imputation schemes or model fits, as shown in Section~\ref{sub:different_aesthetics}. A single data frame is also easier to save than a multitude of vectors, which means it's easier to reproduce your results or send your data to someone else.  
...
\noindent Any change of values or dimensions is legitimate.  However, if a variable changes from discrete to continuous (or vice versa), you will need to update the default scales, as described in Section~\ref{sec:scale-usage}.  
...
Table~\ref{tbl:geoms} lists all of the geoms available in ggplot.  

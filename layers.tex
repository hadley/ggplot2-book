...
% p <- p + geom_point()
% summary(p)
\input{_include/d10b2b494911f964b04b4af25ea0836c.tex}
% END
...
% p %+% mtcars
\input{_include/033b0c35ccd503e755a5fb5a06bfd4ce.tex}
% END

\noindent Any change of values or dimensions is legitimate as long as the variables used in the plot are still part of the data.  This facility can be useful if you need to produce the same plot for different datasets.  It's also an easy way to experiment with influential points or imputation schemes.
...
% p + geom_point()
\input{_include/009f76a18d8f5964151b8b97ddb80fbb.tex}
% END
...
\input{_include/24d4cd9e02152d4816d84418f37bffec.tex}
% END
...
\input{_include/7b5802942c5724cae1e387ff6386508c.tex}
% END
...
% LISTING
% p <- ggplot(Oxboys, aes(x=age, y=height)) + geom_line(aes(group=Subject))
% p
% END
...
\input{_include/61af7a6cf5df6d33d89e98edaba46812.tex}
% END
...
\input{_include/a3c6a45b5992618d718199be47c36ac7.tex}
% END
...
% LISTING
% p <- ggplot(Oxboys, aes(x=Occasion, y=height)) + geom_boxplot(colour=I("#3366FF"), size=I(1))
...
\input{_include/3b10a5f8f568d798a01ae28280121cfe.tex}
% END
...
For lines and paths, operates on an off by one principle.  Blending too complicated in general (how would you blend from green dotted lines to red dashed lines?).  Rules for colour & line type (due to technical limitations in R).
...
% d + stat_bin(aes(y=1, fill = ..count..), binwidth = 0.1, geom = "tile")
% END
...
\input{_include/414f3998f96e54fd7c4633ae95aee5df.tex}
% END
...
% oplot <- ggplot(data=Oxboys, aes(x=age, y=height, group=Subject)) + geom_line()
...
\input{_include/690c7d8c4abb414c36f7b8238cbc409a.tex}
% END
...
\input{_include/8f1a037a36edd080862f33bf8dffc62f.tex}
% END

The smooth line makes it evident that the residuals are not random, showing a deficiency in the model.
We add a quadratic term, refit the model, recalculate predictions and residuals, and replot.  There now much less evidence of model inadequacy.
...
\input{_include/fc60f823bc6348c2ff97aab77924ff08.tex}
% END

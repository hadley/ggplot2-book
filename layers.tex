...
As these two examples show, the default mappings in the plot \code{p} can be extended or overridden in added layers: \index{Mappings!overriding}
...
\paragraph{Overiding the default grouping.}  The plot has a discrete scale but you want to draw lines that connect {\em across} groups.  This is the strategy used in interaction plots, profile plots, and parallel coordinate plots, among others.  For example, we draw boxplots of height at each measurement occasion, as shown in the first figure in Figure~\ref{fig:group-boxplots}: \indexf{geom_boxplot}

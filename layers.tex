...
% dfs: this second item would benefit from an example.
...
To illustrate these ideas, we will look at a simple longitudinal data set, \code{Oxboys} from the \pkg{nlme} package.  It records the heights (\var{height}) and ages (\var{age}) of 26 boys (\var{Subject}) measured on nine occasions (\var{Occasion}). Figure~\ref{fig:group-lines} shows a time series plot for each boy, generated as follows:

% LISTING
% ggplot(Oxboys, aes(x=age, y=height)) + geom_line(aes(group=Subject))
% END

\noindent We specified the group aesthetic, {\tt aes(group = Subject)}, to identify the observations to be connected by line segments.  This was a requirement in this example due to the presence of a second categorical variable, \var{Occasion}.  The second plot in  Figure~\ref{fig:group-lines} shows the result of relying  ...  I'm confused here because I can't figure out what argument produces the plot we see when no grouping variable is specified. ... sent email to hadley.

...
<<duplication, echo=FALSE, message=FALSE>>=
library("ggplot2")
knitr::opts_chunk$set(fig.width = 4, fig.height = 4, fig.align = "center", out.width = "0.49\\linewidth", fig.show = 'hold', message = FALSE, cache = TRUE, cache.path = "cache/duplication-", fig.path = "figure/duplication-")
options(digits = 2)
@
...
<<iterate-limits, dev='png', out.width="0.32\\linewidth", fig.cap="When ``zooming'' in on the plot, it's useful to use \\f{last_plot} iteratively to quickly find the best view. The final plot adds a line with slope 1 and intercept 0, confirming it is the square diamonds that are missing.">>=
...
<<gradient-rb, fig.cap="Saving a scale to a variable makes it easy to apply exactly the same scale to multiple plots.  You can do the same thing with layers and facets too.">>=
...
<<quiet, fig.cap="Using ``quiet'' x and y scales removes the labels and hides ticks and gridlines.">>=
...
<<geom-lm, fig.cap="Creating a custom geom function saves typing when creating plots with similar (but not the same) components.">>=
...
<<pcp, fig.width=8, out.width="0.75\\linewidth", fig.show='asis', prompt=TRUE>>=

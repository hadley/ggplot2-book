...
While it is possible to build up a new theme from the ground it is usually easier and less error-prone to modify an existing theme. This approach is often taken in the ggplot2 source. For example, here is the source code for `theme_minimal()`:
...
base + theme_void()
base + theme_background()
base + theme_background("hotpink4")
```

Note that the ggthemes [@ggthemes] package already contains a `theme_solid()` function that provides essentially the same functionality.



<!-- DJN: Might not be a good idea to provide a how-to guide for bad practice! -->
<!--
### Novel themes

While not generally advisable, it is possible to create a theme without modifying an existing one. The general approach is to use only the `theme()` function while setting `complete = TRUE`. You can find an example of this in the ggplot2 source by looking at the `theme_gray()` function. However, if you do take this approach it is important to ensure that you define every theme elements that do not inherit values from other elements. To determine if a theme element inherits from another one, you can use the `get_element_tree()` function, which returns a list of all theme elements and information about them. Any time the value of `inherits` is `NULL`, it means this element does not inherit from any other:

```{r}
element_tree <- get_element_tree()

# the title element does not inherit
element_tree[["title"]]       

# the plot.title element inherits from title
element_tree[["plot.title"]]
```

Here is a list of theme elements that do not inherit:

```{r}
# for each element in the tree, check if it inherits
inherits <- sapply(element_tree, function(etree) {
  ifelse(test = is.null(etree$inherit), yes = FALSE, no = TRUE)
})

# print the names of elements that do not inherit
names(element_tree[!inherits])
```

-->

<!-- ### Adding to the element tree -->

<!-- In the previous section I introduced the concept of the "element tree", which describes the relationship between different elements, and specifies what class each element is (e.g., `"element_text"`, `"element_line"`, etc). -->

### New elements

It is also possible to create new theme elements. In Section \@ref(titles), for example, we encountered the `element_markdown()` function supplied by the ggtext package [@ggtext] ...

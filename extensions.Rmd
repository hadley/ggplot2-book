...
When creating new stats it is usually a good idea to provide an accompanying `geom_*()` constructor as well as the `stat_*()` constructor, because most users are accustomed to adding plot layers with geoms rather than stats. For our convex hull example, this constructor might look like this:
...
geom_chull <- function(mapping = NULL, data = NULL, stat = "chull",
                       ...
                       inherit.aes = TRUE, ...) {
  layer(
    geom = GeomPolygon, data = data, mapping = mapping, stat = stat, 
    position = position, show.legend = show.legend, inherit.aes = inherit.aes,
    params = list(na.rm = na.rm, ...)
  )
}
...
Now we can create plots using code like this:

```{r}
ggplot(mpg, aes(displ, hwy)) + 
  geom_point() + 
  geom_chull(fill = NA, colour = "black")
```

Note that it is not always possible to define `geom_*()` constructor in a sensible way, as can happen when there is no obvious default geom for the new stat (e.g., if it is unclear if `geom_chull()` should rely on `GeomPolygon` or `GeomPoint`), or if the stat is intended to offer a slight modification to an existing geom/stat pair. In such cases it may be wise to provide only a `stat_*()` function. 

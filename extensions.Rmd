...
<!-- DJN: not sure how much printing the source helps here? Maybe better to explain in next chapter -->
<!-- ```{r} -->
...
To give a simple example, I'll implement a slightly simplified version of the `position_jitternormal()` function from the ggforce package, which behaves in the same way as `position_jitter()` except that the perturbations are sampled from a normal distribution rather than a uniform distribution. In order to keep the exposition simple, I'll assume we have the following convenience function defined:
...
In a realistic implementation, the `position_jitternormal()` constructor would apply some input validation to make sure the user has not specified negative standard deviations, but in this context I'll keep it simple:
...
To illustrate how you can create new facets by subclassing an existing facet, I'll create a relatively simple facetting system that "scatters" the panels, placing them in random locations on a grid. To do this I'll create a new ggproto object called `FacetScatter` that is a subclass of `FacetWrap`, and write a new `compute_layout()` method that places each panel in a randomly chosen cell of the panel grid:

```{r}
FacetScatter <- ggproto("FacetScatter", FacetWrap,
  
  # this isn't important to the example: all I'm doing is

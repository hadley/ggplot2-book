...
You can generate many plots without knowing how scales work, but understanding scales and learning how to manipulate them will give you much more control. The basics of working with scales is described in [scale usage](#scale-usage). [Guides](#guides) discusses the common parameters that control the axes and legends. Legends are particularly complicated so have an additional set of options as described in [legends](#legends). [Limits](#limits) shows how to use limits to both zoom into interesting parts of a plot, and to ensure that multiple plots have matching legends and axes. [Scale details](#scale-details) gives an overview of the different types of scales available in ggplot2, which can be roughly divided into four categories: continuous position scales, colour scales, manual scales and identity scales. 
...
A number of settings that affect the overall display of the legends are controlled through the theme system. You'll learn more about that in [themes](#themes), but for now, all you need to know is that you modify theme settings with the `theme()` function. \index{Themes!legend} 
...
In either case, the transformation occurs before any statistical summaries. To transform, _after_ statistical computation, use `coord_trans()`. See [cartesian coordinate systems](#cartesian) for more details.
...
See [multiple time series](#spread-gather) for another approach.

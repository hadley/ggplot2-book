...
Formally, each scale is a function from a region in data space (the domain of the scale) to a region in aesthetic space (the range of the scale). The domain of each scale corresponds to the range of the variable supplied to the scale, and can be continuous or discrete, ordered or unordered. The range consists of the concrete aesthetics that you can perceive and that R can understand: position, colour, shape, size and line type. 
...
*   __training__:  During this key stage, the domain of the scale is learned from
    the data. Sometimes learning the domain of a scale is extremely 
    ...
    categorical variable.  However, often the domain must reflect multiple layers 
    across multiple datasets in multiple panels.  For example, imagine a scale 
    that will be used to create an axis; the minimum and maximum values of the 
    raw data in the first layer and the statistical summary in the second 
    layer are likely to be different, but they must all eventually be drawn on 
    the same plot. \index{Scales!training}
...
Every aesthetic can use an identity or manaul scale.
...
The following code illustrates this process.  We start with a plot that uses the default colour scale, modify it to adjust the appearance of the legend, and then use a different colour scale:

```{r scale-adjust}
p <- ggplot(msleep, aes(sleep_total, sleep_cycle)) + 
...
*   `name`:  sets the label which will appear on the axis or legend. You can 
    supply text strings (using `\n` for line breaks) or mathematical expressions 
    (as described by `?plotmath`).  \index{Axis!labels} \index{Legend!title} 
    \index{Scales!names}
...
*   `limits`: fixes the domain of the scale. Continuous scales take a numeric 
    vector of length two; discrete scales take a character vector. If limits are 
    set, no training of the data will be performed. See 
    [position scales](#sub:scale-position) for shortcuts. \index{Axis!limits}
    \index{Scales!limits}
    ...
    for ensuring that limits are consistent across multiple plots intended to be 
    compared (i.e., setting limits that are larger than some of the default 
    ranges).  

*   `breaks` and `labels`: `breaks` controls which values appear on the axis or 
    legend, i.e., what values tick marks should appear on an axis or how a 
    continuous scale is segmented in a legend. `labels` specifies the labels 
    that should appear at the breakpoints. If `labels` is set, you must also 
    specify `breaks`, so that the two can be matched up correctly. 
    \index{Axis!breaks} \index{Axis!labels} \index{Legend!keys}
    
    `breaks` and `labels` can be NULL, character vector, function...
    ...
    p
    p + scale_x_continuous(breaks = c(2, 4))
...
*   `na.value`: where possible, ggplot2 tries to keep missing values in the 
    plot, so they don't go silently missing. This means that many scales try
    to pick a useful default for missing values.
...
*   `oob`: By default, any value not in the domain of the scale is
    replaced with a missing value. You can override with the `oob` 
    (out of bounds) argument to the scale. The default is `scales::censor()` 
    which replaces any value outside the limits with `NA`. Alternatively, you 
    could use `scales::squish()` which squishes all values into the range.
...
Every plot must have two position scales, one for the horizontal position (the x scale) and one for the vertical position (the y scale). ggplot1 comes with continuous, discrete (for factor, character and logical vectors) and date scales. Each of these transform the data in a slightly different way, and generate a slightly different type of axis. The following sections describe each type in more detail. \index{Scales!position} \index{Positioning!scales}
...
Like all other scales, the `breaks`, `minor_breaks` and `labels` arguments control the appearance and location of the ticks. However, there are two special functions available for dates that generate useful breaks and labels:
...
ggplot(df, aes(trt, value)) + 
  geom_point() + 
  geom_text(aes(as.numeric(trt) + 0.1, label = format(value)))

ggplot(df, aes(trt, value)) + 
  geom_point() + 
  geom_text(aes(label = format(value)), position = position_nudge(x = 0.1))
```

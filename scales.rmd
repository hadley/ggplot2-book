...
<!-- TODO
  This last sentence is complicated, and I think an example would help demonstrate, or at least mention that this is revisted in [Continuous position scales](#sub:scale-position) ^^^
-->
...
ggplot(df, aes(y, y)) + 
  geom_point(size = 6, colour = "grey20", show.legend = TRUE) +
  geom_point(aes(colour = z), size = 3) 
```
...
p + guides(fill = guide_colorbar(barheight = unit(3, "cm")))
```
...
Date and date time data are continuous variables with special labels. ggplot2 works with `Date` (for dates) and `POSIXct` (for date times) classes: if your dates are in a different format you will need to convert them with `as.Date()` or `as.POSIXct()`. \index{Date} \index{Time} \index{Scales!date-time} \indexf{scale\_x\_datetime} `scale_x_date()` and `scale_x_datetime()` work similarly to `scale_x_continuous()` but have special `date_breaks` and `date_labels` arguments that work in date-friendly units:
...
*   The default colour scheme, `scale_colour_hue()`, picks evenly spaced hues 
    ...
    \index{Colour!discrete scale} \indexf{scale\_colour\_hue}
...
*   `scale_colour_manual()` is useful if you have your own discrete colour
    ...
    Wes Anderson movies, as provided by the WesAnderson package. These

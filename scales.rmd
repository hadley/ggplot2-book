...
* Continious position scales used to map integer, numeric, and date time 
...
Date and date time data are continuous variables with special labels. ggplot2 works with `Date` (for dates) and `POSIXct` (for date times) classes: if your dates are in a different format you will need to convert them with `as.Date()` or `as.POSIXct()`. \index{Date} \index{Time} \index{Scales!date-time} \indexf{scale_x_datetime} `scale_x_date()` and `scale_x_datetime()` work similarly to `scale_x_continuous()` but have different defaults for `breaks` and `labels`. You can override the defaults with:

*   `breaks = scales::date_breaks(width)` allows you to position breaks by
    date units (years, months, weeks, days, hours, minutes, and seconds). 
    For example, `breaks = scales::date_breaks("2 weeks")` will place a major 
    tick mark every two weeks.

*   `labels = scales::date_labels(format)` controls the display of the labels
...
base + scale_x_date(
  ...
  minor_breaks = scales::date_breaks("2 weeks")
)
```
...
*   `scale_colour_gradient()` and `scale_fill_gradient()`: a two-colour gradient, 
    low-high (light blue-dark blue). This is the default scale for continous
    ...
    \indexf{scale_colour_gradient} \indexf{scale_fill_gradient}
...
*   `scale_colour_gradientn()` and `scale_fill_gradientn()`: a custom n-colour
    ...
    scales. \index{Colour!palettes} \indexf{scale_colour_gradientn}
    \indexf{scale_fill_gradientn}
    ...
    make them unevenly spaced, use the`values` argument, which should be between 
    0 and 1 if `rescale` is true (the default), or within the range of the data 
    is `rescale` is false.
...
There are four colour scales for discrete data. We'll illustrate them with a barchar that encodes both position and fill to the same variable:
...
*   The default colour scheme, `scale_colour_hue()`, picks evenly spaced hues 
    ...
    \index{Colour!discrete scale} \indexf{scale_colour_hue}
...
What you need to do is give the lines informatives labels:
...
See [multiple time series](#sub:time-series) for another approach.

### The identity scale {#sub:scale-identity}

The identity scale is used when your data is already scaled, when the data and aesthetic spaces are the same. \index{Scales!identity} \indexf{scale_identity}

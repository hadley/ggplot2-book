...
Every aesthetic has a default scale that is added to the plot whenever you use that aesthetic. These are listed in Table \ref{tbl:default-scales}. The scale depends on the variable type: continuous (numeric) or discrete (factor, logical, character). If you do not want a default scale, you can overwrite it, as described in [scales](#sub:customise-scales).
...
The following code illustrates this process.  We start with a plot that uses the default colour scale, modify it to adjust the appearance of the legend, and then use a different colour scale.  The results are shown in Figure \ref{fig:scale-adjust}.
...
This section describes each group in more detail.  Precise details about individual scales can be found in the documentation, within R (e.g., `?scale_fill_discrete`), or online at  <http://docs.ggplot2.org/>.
...
Of course, you can also perform the transformation yourself. For example, instead of using `scale_x_log10()`, you could plot `log10(x)`. That produces an identical result inside the plotting region, but the axis and tick labels won't be the same. If you use a transformed scale, the axes will be labelled in the original data space. In both cases, the transformation occurs before the statistical summary. Figure \ref{fig:trans} illustrates this difference with the following code. \index{Transformation!data}
...
```{r date-scale, out.width="0.32\\linewidth", fig.cap="A time series of personal savings rate. (Left) The default appearance, (middle) breaks every 10 years, and (right) scale restricted to 2004, with YMD date format. Measurements are recorded at the end of each month."}
...
```{r gradient, out.width="0.32\\linewidth", dev='png', fig.cap="Density of eruptions with three colour schemes. (Left) Default gradient colour scheme, (middle) customised gradient from white to black and (right) 3 point gradient with midpoint set to the mean density."}
...
```{r brewer-pal, out.width="0.32\\linewidth", fig.cap="Three ColorBrewer palettes, Set1 (left), Set2 (middle) and Pastel1 (right), applied to points (top) and bars (bottom). Bright colours work well for points, but are overwhelming on bars. Subtle colours work well for bars, but are hard to see on points."}
...
```{r scale-manual, out.width="0.32\\linewidth", fig.cap="Scale manual used to create custom colour (left and middle) and shape (right) scales."}
...
```{r huron, out.width="0.75\\linewidth", fig.width=7, prompt=TRUE, fig.align='left'}
...
```{r huron2, out.width="0.75\\linewidth", fig.width=7, prompt=TRUE, fig.align='left'}
ggplot(huron, aes(year)) +
...
```{r huron3, out.width="0.75\\linewidth", fig.width=7, prompt=TRUE, fig.align='left'}
ggplot(huron, aes(year)) +
  geom_line(aes(y = level + 5, colour = "above")) + 
  geom_line(aes(y = level - 5, colour = "below")) + 
...
```{r scale-identity, echo=FALSE, out.width="0.5\\linewidth", fig.cap="A plot of R colours in Luv space.  A legend is unnecessary, because the colour of the points represents itself: the data and aesthetic spaces are the same."}
x <- colors()
...
```{r legend-geom, echo=FALSE, fig.width = 4, fig.height = 2, out.width = "3.5in", fig.cap="Legends produced by geom: point, line, point and line, and bar."}
...
```{r legend-merge, echo=FALSE, fig.width = 4, fig.height = 2, out.width = "3in", fig.cap="Colour legend, shape legend, colour + shape legend."}

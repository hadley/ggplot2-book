...
Scales control the mapping from data to aesthetics. They take your data and turn it into something that you can see, like size, colour, position or shape. Scales also provide the tools you use to read the plot: the axes and legends. Formally, each scale is a function from a region in data space (the domain of the scale) to a region in aesthetic space (the range of the scale). The guide or legend is the inverse of a scale: it allows you to read data values off the plot.
...
Default scales and how to override them are described in [scale usage](#sec:scale-usage). 
...
Without a scale, there is no way to go from the data to aesthetics, so a scale is required for every aesthetic used on the plot. 
...
To modify a scale, you override the default scale and supply additional arguments.  The parameters of a scale that you're most common modify are those that control the appearance of the guide, as described below.

### Exercises

1.  What happens if you connect a discrete variable to a continuous scale?
    What happens if you connect a continuous variable to a discrete scale?

## Gudes: legends and axes {#sec:guides}

The component of a scale that you're most likely to want to modify is the __guide__, the axis or legend associated with the scale. Guides allow you to read observations from the plot and map them back to their original values. If you blinked when you read that both axes and legends are instances of the general class, you are not alone. The notion that the same kind of object is used to map data to positions and symbols strikes some people as unintuitive. However, there are natural equivalents between the legend and the axis: the legend title and axis label are equivalent and determined by the scale name; the legend keys and tick labels are both determined by the scale breaks. \index{Guides} \index{Legend}
...
Each of these parameters is control by an argument to the `scale_` function. Every scale has these elements, so every scale function has these three arguments:

| Scale parameter | Axis              | Legend
|-----------------|-------------------|---------------              
| `name`          | Label             | Title
...
There are two ways to remove the axis label. Setting it to `""` removes it, but still allocates space; `NULL` removes the label and its space. Look closely at the left and bottom borders of these plots. I've drawn a grey rectangle around the plot to make it easier to see the difference.
...
Other useful labelling functions:
...
While the most important parameters are shared between axes and legends, there are two important ways that legends are different from axes:
...
df$z <- cut(df$x, 3, labels = c("a", "b", "c"))
ggplot(df, aes(x, y)) + 
...
The position and justification of legends are controlled by the theme setting `legend.position`, and the value can be "right", "left", "top", "bottom", "none" (no legend). \index{Legend!position}
...
Switching between left/right and top/bottom modifies how each legend is laid out (horizontal or vertically), and how multiple legends are stacked (horizontal or vertically). If needed, you can adjust those options independently:
...
Alternatively, you can give a numeric vector of length two (with values between 0 and 1) the position of the corner given by `legend.justification`, a numeric vector of length two. Top right = `c(1, 1)`, bottom left = `c(0, 0)`. This can be useful if there's a lot of blank space in your plot, but positioning the legend exactly where you want it requires a lot of trial and error.
...
base + theme(legend.position = c(0.35, 1), legend.justification = c(1, 1))
base + theme(legend.position = c(0.5, 0.5), legend.justification = c(0.5, 0.5))
```

### Guide functions

You can get additional control over the fine details of the display of the guide with `guide_colourbar()` and `guide_legend()`. Legend guides can be used for any aesthetic (discrete or continuous) while the colour bar can only be used with continuous colour scales. 
...
Both functions have numerous examples in their documentation help pages that illustrate each argument at least once. Below, I'll describe the most important pieces that you're likely to want to change. Most of the arguments to the guide function control the fine level details of the text colour, size, font etc. You'll learn about those in the themes chapter.

#### `guide_legend()`

The legend guide displays one key per element. The most useful options are:

*   `nrow` or `ncol` which set the number of rows or columns in the legend. 
    `byrow` controls how the elements are filled into the table: `FALSE` fills 
    the table by column (the default), `TRUE` fills it by row
...
*   `reverse`: this is particularly useful when you have stacked bars because 
    the default stacking and legend orders are different:
...
The colour bar guide is designed for continuous ranges of colors---as its name implies, it outputs a rectangle over which the color gradient varies. 
...
* `reverse` flip the colour bar to go from highest to lowest.
...
p + guides(fill = guide_colorbar(barheight = 3, default.unit = "cm"))
```
...
Every plot must have two position scales, one for the horizontal position (the x scale) and one for the vertical position (the y scale). \index{Scales!position} \index{Positioning!scales} 

There are three types of position scales: 
...
Because it is often useful to place labels and other annotations on intermediate positions on the plot, discrete position scales also accept continuous values.  You can use `position_nudge()` to tweak the position of labels. \index{Scales!position!discrete} \indexf{scale_x_discrete} \indexf{scale_y_discrete} \index{Scales!discrete}

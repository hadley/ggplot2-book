...
There are four types of map data you might want to visualise: vector boundaries, point metadata, area metadata, and raster images. Typically, assembling these datasets is the most challenging part of drawing maps. Unfortunately ggplot2 can't help you with that part of the analysis, but I'll provide some hints about other R packages that you might want to look at. I'll illustrate each of the four types of map data with some maps of Michigan.
...
The reason why the data frame contains separate `group` and `id` is that some  geographical units cannot be specified using a single contiguous polygon. A goo
...
(Note that finding the appropriate `scale` required a lot of manual tweaking). Once you have the raster data, you can use the `ggmap()` function from the
ggmap package to create a plot showing the raster map (left panel). The output
from this function is a ggplot object, so you can add layers to it in the 
usual fashion to overlay the map with the desired metadata (right panel):
...
For more information about mapping using simple features, see Geocomputation with R <https://geocompr.robinlovelace.net/> by Robin Lovelace, Jakub Nowosad and Jannes Muenchow. 

...
ggmap package to create a plot showing the raster map (left panel below). 
...
Just like vector maps, a raster map can be linked to metadata representing an additional information source. The output
from this function is a ggplot object, so you can add layers to it in the 
usual fashion to overlay the map with the desired metadata (right panel above)

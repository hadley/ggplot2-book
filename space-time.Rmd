...
ggplot(faithfuld, aes(eruptions, waiting)) + 
  geom_contour(aes(z = density, colour = stat(level)))
```

The reference to the `stat(level)` variable in this code may seem confusing, because there is no variable called `level` in the `faithfuld` data. In this context the `stat()` notation refers to a variable computed internally (see Section \@ref(generated-variables)). To display the same density as a heat map, you can use `geom_raster()`:

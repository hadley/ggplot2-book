...
*   `family` gives the name of a font. There are only three fonts that
    ...
    * `showtext` makes GD-indpendent plots by rendering all text as polygons. 
...
*   `size` controls the size of font. Unusually, it's measured in mm, rather
...
Labels can also be used as alternative to a legend. This generally makes the plot easier to read because it puts the lookup table closer to the data. The directlabels package provides a number of tools to make this easier:
...
ggplot(mpg, aes(displ, hwy, colour = class)) + 
  directlabels::geom_point(show_guide = FALSE) +
...
A a data frame with one row for each "corner" of boundaries of a geographical region like a country, state, or county. It requires four variables:
...
The following code extracts that data from the built in maps package using `ggplot2::map_data()`. The maps package isn't particularly accurate or up-to-date, but it's built in to R so it's a reasonable place to start.
...
Sometimes metadata is assocaited not with a point, but with an area. For example, we can create `mi_census` which provides census information about each county in MI:

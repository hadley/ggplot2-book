...
We'll then take a brief break and introduce the larger diamonds dataset in [diamonds](#sec:diamonds). The final three sections use this dataset to illustrate techniques for visualising larger datasets:
...
The chapter concludes with some pointers to other packages, built on top of ggplot2, that provide additional layers.
...
*   `geom_line()` makes a __line plot__.  The `group` aesthetic determines 
    ...
    detail. `geom_path()` is similar to a `geom_line()`, but lines are 
...
From ggplot2's perpsective, the "width" and "height" of a label are 0, so you can't stack or dodge labels, and axis limits are not automatically expanded to include all text. Obviously, labels do have height and width, but they are physical units, not data units. The amount of space they occupy on that plot is not constant in data units: when you resize a plot, the labels stay the same size, but the size of the axes changes. This means that good labels often requires some manual tweaking of the axis limits.
...
*   `family` gives the name of a font. There are only three fonts that
    ...
    * extrafont, <https://github.com/wch/extrafont>, by Winston Change, 
...
*   You can adjust the alignment of the text with the `hjust` ("left", "center", 
    ...
    of the plot: \index{Text!justification}
...
*   If `check_overlap = TRUE`, overlapping labels will be automatically 
    ...
    it's omitted. \indexc{check\_overlap}
...
A variation on `geom_text()` is `geom_label()`: it draws a rounded rectangle behind the text. This makes it useful for adding labels to plots with busy backgrounds: \indexc{geom\_label}
...
ggplot(faithfuld, aes(waiting, eruptions)) +
  ...
  geom_label(data = label, aes(label = label), alpha = 0.25)
```
...
* `geom_text()` to add text descriptions or to label points.  Most plots 
  ...
  \index{Labels} \indexc{geom\_text}
...
ggplot2 does not support true 3d surfaces. However, it does support many common tools for representing 3d surfaces in 2d: contours, coloured tiles and bubble plots. These all work similarly, differing only in the aesthetic used for the 3d dimension. \index{Surface plots} \index{Contour plot} \indexc{geom\_contour}
...
For interactive 3d plots, including true 3d surfaces, see RGL, <http://rgl.neoscientists.org/about.shtml>. \index{3d graphics} 
...
*   Bin the points and count the number in each bin, then visualise that count 
    ...
    ```{r overp-bin}
    norm + stat_bin2d()
    norm + stat_bin2d(bins = 10)
    ```
    
    ```{r overp-bin-hex}
    norm + stat_binhex()
    norm + stat_binhex(bins = 10)
    ```
...
`stat_bar()`, `stat_bin()`, `stat_bin2d()`, and `stat_binhex()` summarise the data by dividing into bins and counting the number of observations in each bin. `stat_summary()`, `stat_summary_bin()`, `stat_summary_2d()`, and `stat_summary_hex()` generalise this idea to allow you to summarise an additional variable using the function of your choice. 

\index{Summary!statistical} \indexf{stat\_summary} \index{Stats!summary}
...
`stat_summary()` is designed to show the summary in the `y` aesthetic; `stat_summary_2d()` and `stat_summary_hex()` are designed to show the aesthetics in the fill. That makes `stat_summary()` a bit more flexible because it can also use the `ymin` and `ymax` aesthetics with interval geoms. You can either supply (up to) 3 individual functions with `fun.ymin`, `fun.y` and `fun.ymax` or a single summary function that returns a named vector with `fun.data`.

...
* Working with __collective__ geoms that are composed of multiple observations
  ([working with groups](#sec:grouping))
...
Each of these geoms is two dimensional and requires both `x` and `y` aesthetics. All understand `colour` (or `color`) and `size` aesthetics, and the filled geoms (bar, tile and polygon) also understand `fill`. The geoms are used for displaying data, summaries computed elsewhere, and metadata.
...
*   `geom_line()` makes a __line plot__.  The `group` aesthetic determines 
    ...
    lines. \index{Line plot} \indexf{geom_line} \indexf{geom_path}
    
*   `geom_point()` produces a __scatterplot__. `geom_point()` also understands
    the `shape` aesthetic. \indexf{geom_point}
...
*   `geom_tile()` and `geom_raster()` make an image plot (aka a level plot). 
    ...
    size. \index{Image plot} \index{Level plot} \indexf{geom_tile}
...
`geom_text()` adds labels at the specified points. It requires the `x`, `y` and `label` aesthetics.
...
draw.text <- function(just, i, j) {
  grid.points(x[j], y[i], gp = gpar(fill = "grey50"), pch = 20, size = unit(0.5, "char"))
  abcd <- textGrob(
    "ABCD", 
    x = x[j], y = y[i], 
    just = just, 
    gp = gpar(fontsize = 16, colour = "black")
  )
  
  grid.draw(abcd)
  grid.text(
    deparse(substitute(just)), 
    x = grobX(abcd, "north"), y = grobY(abcd, "north") + unit(0.5, "lines"), just = c(0.5, 0),
    gp = gpar(col = "grey30", fontsize = 8, fontface = "bold")
  )
}

grid.newpage()
grid.rect(gp= gpar(fill="grey93", col=NA))
pos <- c(0.2, 0.5, 0.8)
x <- unit(pos, "npc")
y <- unit(pos, "npc")
...
*   Horizontal and verticle alignment: `hjust` ("bottom", "middle", "top", 
    "inward", "outward") and `vjust` ("left", "center", "right", "inward", 
    "outward")

*   `size` in mm. Normally fonts are measured in pt. There are 2.83 points per
    milliment, so to convert from points to mm, just divide by 2.83.

*   `family` = name of a font family. The only families guaranteed to work on
    every device are "serif", "sans" and "mono". Font selection in R is a complex 
    topic as is it peformed per graphic device so the fonts supported by the 
    on screen display (`quartz()` on mac, `X11` on linux, and `windows()` on
    windows), may be different to the fonts supported for pdf and png output.

    There are two packages that simplify the quandary a bit. `showtext` makes 
    graphics-device-indpendent plots by rendering all text as polygons.
    `extrafont` takes an alternative approach, converting fonts to a standard
    format that all devices can use. Both approaches have pros and cons, so
    you may need to try them both out and see which works best for your needs.
    
*   `fontface` = "bold", "italic", or "plain" (the default).
...
*   Often you want to label existing points on the plot. You don't want the
    ...
    nudge the text a little horizontally or vertically, without having to
    modify the default aesthetics (and are certainly easier if you have
    categorical axes.)

### Limits

Unfortunately text labels don't affect the limits of the plot. This is because they have absolute sizes (e.g. a label might be 3cm), regardless of the size of the plot. This means there's no way for ggplot2 to reliably figure out the correct size.
...
You can also directly label the data instead of relying on the legend. The directlabels package makes this particularly easy:

```{r, eval = FALSE}
library(directlabels)
ggplot(mpg, aes(displ, hwy, colour = class)) + 
  geom_point(show_guide = FALSE) +
  geom_dl(aes(label = class), list("last.points", rot = 30), show_guide = FALSE) 

ggplot(mpg, aes(displ, hwy, colour = class)) + 
  geom_point(show_guide = FALSE) +
  geom_dl(aes(label = class), "smart.grid", show_guide = FALSE)
```
...
This is not what we wanted; we have inadvertently added a smoothed line for each boy. Grouping controls both the diplsay of the geoms, and the operation of the stats: one statistical transformation is run for each group.

Instead of setting the grouping aesthetic in `ggplot()`, where it will apply to all layers, we set it in `geom_line()` so it applies only the lines. There are no discrete variables in the plot so the default grouping variable will be a constant and we get one smooth:
...
For 1d continuous distributions the most important geom is the histogram, `geom_histogram()`: \indexf{geom_histogram} 
...
* Use colour and a frequency polygon, `geom_freqpoly()` . 
  \index{Frequency polygon} \indexf{geom_freqpoly}
...
The histogram, frequency polygon and density display a detailed view of the distribution. However, sometimes you want to compare many distributions, and it's useful to have alternative displays sacrify quality for quantity. Here are three options:
...
*   If there is some discreteness in the data, you can randomly jitter the 
    ...
    nearest integer, so we set a jitter width of half of that to smear away the 
    discrete. \index{Jittering} \indexf{geom_jitter}
...
There are four types of map data you might want to visualise: vector boundaries, raster background, point metadata and area metadata. Typically assmebling these datasets is the most challenging part of generating spatial visualisations. Unforuntately ggplot2 can help you with that part of the analysis, but I'll provide some hints about other R packages that you might want to look at.

I'll illustrate each below with an example for information about Michigan.

* `mi_counties` contains information about the boundaries of each county. There
   are four key variables: `lat` and `long` (the position), `group` (a unique 
   identifier for each polygon), and `county` (the name of the county, needed
   for matching up with other data)`. (Note that we need both `group` and 
   `county` variables because in principle a county might be made up of more
   than one continguous piece.). This is known as vector data

    `r columns(1, 2/3, 2/3)`
    ```{r}
    mi_counties <- map_data("county", "michigan") %>% rename(county = subregion)
    head(mi_counties)
    ```
    
    We can visualise with `geom_polygon()`:
...
*  A _raster_ map from OpenStreet maps. A raster map is a matrix of coloured
   pixels. The easiest way to get a raster map of a given area is to use
   the ggmap package. Downloading the raster data is often time consuming 
   so it's a good idea to cache it in a rds file.
    ...
    You can display it with `ggmap()`:
    
    ```{r}
    ggmap::ggmap(mi_raster)
    ```
    
*   `mi_cities()` contains some information about the biggest cities in MI 
    (as of 2006).

    ```{r}
    data("us.cities", package = "maps")
    mi_cities <- filter(us.cities, country.etc == "MI")
    ```
    ...
    ```{r}
    ggplot(mi_cities, aes(lat, long)) + 
      geom_point(aes(size = pop)) + 
      scale_size_area() + 
      coord_quickmap()
    ```
    
*   Finally, we have some demographic information about each county:
    ...
    We don't have an easy way to visualise this yet.
    
Drawing maps is just a matter of combining these different datasets.
...
ggmap::ggmap(mi_raster) + 
  geom_point(aes(long, lat, size = pop), mi_cities, colour = "red") + 
  scale_size_area()
```

To visualise county demographics, we need to combine `mi_counties` with `mi_cenus`. The easiest way is with `left_join()`:

`r columns(1, 2/3)`
```{r}
census_counties <- left_join(mi_census, mi_counties)
ggplot(census_counties, aes(long, lat, group = county)) + 
  geom_polygon(aes(fill = poptotal)) + 
  coord_quickmap()
```

Typically, the biggest challenge with mapping is finding a good source data, and then ensuring that you match your data with the map data. Some packages that might help are:
...
* `USAboundaries`: this provides state and county boundaries from 1629-2000.

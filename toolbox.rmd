...
* Displaying distributions, continuous and discrete, 1d and 2d, joint and conditional ([link to section](#sec:distributions)).
...
* Statistical summaries display informative data summaries ([link to section](#sec:summary))
* Drawing maps ([link to section](#sec:maps)).
* Revealing uncertainty and error, with various 1d and 2d intervals ([link to section](#sec:uncertainty)).
* Annotating a plot to label, describe and explain with supplemental information [link to section](#sec:annotating).
...
* `geom_bar(stat = "identity")` makes a __barchart__. We need `stat = "identity"` because the default stat automatically counts values (so is essentially a 1d geom, see [displaying distributions](#sec:distributions). The identity stat leaves the data unchanged. \index{Barchart} \indexf{geom_bar}
...
* `geom_tile()` makes an image plot or level plot.  The tiles form a regular tessellation of the plane and typically have the `fill` aesthetic mapped to another variable. \index{Image plot} \index{Level plot} \indexf{geom_tile}

Each of these geoms is illustrated in Figure \ref{fig:geom-basic}, created with the code below.

```{r geom-basic, fig.width=3, fig.height=3, fig.show='hold', out.width="0.24\\linewidth", fig.cap="The basic geoms applied to the same data. Many give rise to named plots (from top left to bottom right): scatterplot, bar chart, line chart, area chart, path plot, labelled scatterplot, image/level plot and polygon plot. Observe the different axis ranges for the bar, area and tile plots: these geoms take up space outside the range of the data, and so push the axes out."} 
df <- data.frame(
  x = c(3, 1, 5), 
  y = c(2, 4, 6), 
  label = c("a","b","c")
...
```{r geom-boxplot, fig.show='hold', fig.cap="The boxplot geom can be of use to see the distribution of a continuous variable conditional on a discrete varable like cut (left), or continuous variable like carat (right). For continuous variables, the group aesthetic must be set to get multiple boxplots. Here \\texttt{group = plyr::round\\_any(carat, 0.1, floor)} is used to get a boxplot for each 0.1 carat bin."} 
qplot(cut, depth, data=diamonds, geom="boxplot")
qplot(carat, depth, data=diamonds, geom="boxplot", 
      group = plyr::round_any(carat, 0.1, floor), xlim = c(0, 3))
```
  
* `geom_jitter()` = `position_jitter()` + `geom_point()`: a crude way of looking at discrete distributions by adding random noise to the discrete values so that they don't overplot.  An example is shown in Figure \ref{fig:geom-jitter} created with the code below. \index{Jittering}  \indexf{geom_jitter}  \indexf{position_jitter}
...
## Displaying groups {#sub:grouping}
...
The `diamonds` dataset consists of price and quality information for about 54,000 diamonds. You can access it by loading ggplot2:
...
The dataset has not been well cleaned, so as well as demonstrating interesting facts about diamonds, it also shows some data quality problems. We'll also use another dataset, `dsmall`, for plots that are more appropriate for smaller datasets. `dsmall` is a random sample of 100 diamonds from the full `diamonds` dataset.
...
```{r overp-glyph, dev='png', fig.show='hold', out.width="0.32\\linewidth", fig.cap="Modifying the glyph used can help with mild to moderate overplotting.  From left to right: the default shape, \\texttt{shape = 1} (hollow points), and \\texttt{shape= \".\"} (pixel points)."} 
...
* Bin the points and count the number in each bin, then visualise that count in some way (the 2d generalisation of the histogram).  Breaking the plot into many small squares can produce distracting visual artefacts.  [@carr:1987] suggests using hexagons instead, and this is implemented with `geom_hexagon()`, using the capabilities of the __hexbin__ package [@hexbin].  Figure \ref{fig:overp-bin} compares square and hexagonal bins, using parameters `bins` and `binwidth` to control the number and size of the bins.  The complete code is shown below. \index{Binning!1d|see{Histogram}} \index{Binning!2d}  \indexf{geom_hexagon}  \indexf{stat_binhex} \indexf{stat_bin2d}
  
```{r overp-bin, dev='png', out.width="0.32\\linewidth", fig.show='hold', fig.cap="Binning with, top row, square bins, and bottom row, hexagonal bins. Left column uses default parameters, middle column \\texttt{bins = 10}, and right column \\texttt{binwidth = c(0.02, 200)}. Legends have been omitted to save space."} 
...
```{r iowa, prompt=TRUE, fig.align='left'} 
...
% 
% It is often useful to connect related observations explicitly with a line.  This is the basis of the parallel coordinates plot [@inselberg:1985,wegman:1990], the m-n plot [@diaconis:1983], time series plots, and profile plots.  
%
...
`fun.data` can be used with more complex summary functions such as one of the summary functions from the **Hmisc** package [@hmisc] described in Table \ref{tbl:hmisc}. You can also write your own summary function. This summary function should return a named vector as output, as shown in the following example.
...
```{r umep, fig.height=3, fig.width=8, out.width="\\linewidth", prompt=TRUE, fig.align='left', fig.show='asis'} 
...
<!--
% The `weight` aesthetic is also useful when you have weighted data.  All ggplot2 statistics know how to correctly deal with (WHAT TYPE OF WEIGHTS?) weights, which makes plotting your weighted data easy.  For a more comprehensive description of plotting weights, see ``Weight and see''.
-->

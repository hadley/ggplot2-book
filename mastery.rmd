...
In order to unlock the full power of ggplot2, you'll need to master the underlying grammar. By understanding the grammar, and how its components fit together, you can create a wider range of visualizations, combine multiple sources of data, and customise to your heart's content.  You may want to skip this chapter in a first reading of the book, returning when you want a deeper understanding of how all the pieces fit together.
...
knitr::kable(head(scaled, 10), digits = 3, align = "l")
```
...
<!-- TODO
  Is this supposed to be a figure caption?
-->
...
A coordinate system, or **coord** for short, maps the position of objects onto the plane of the plot. Position is often specified by two coordinates $(x, y)$, but potentially could be three or more (although this is not implemented in ggplot2). The Cartesian coordinate system is the most common coordinate system for two dimensions, while polar coordinates and various map projections are used less frequently. \index{Coordinate systems!introduction}
...
`r columns(3)`
```{r coord, eval = FALSE, echo = FALSE, fig.cap = "Examples of axes and grid lines for three coordinate systems: Cartesian, semi-log and polar. The polar coordinate system illustrates the difficulties associated with non-Cartesian coordinates: it is hard to draw the axes well."}
...
p <- ggplot(df, aes(x1, y1)) +
  ...
  theme_linedraw()
p 
p + coord_trans(ytrans = "log")
p + coord_polar()
```

<!-- TODO
  Get the code above working ^^^. Perhaps I should file an issue since this doesn't work? Note examples on ?coord_polar are giving similar errors.
  
  df <- data.frame(x1 = c(1, 10), y1 = c(1, 5))
  p <- ggplot(df, aes(x1, y1)) + geom_point()
  p + coord_polar()
  
#>  Error in element_grob.element_text(el, ...) : 
#>  unused argument (default.units = "native")
-->

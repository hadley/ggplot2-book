...
This chapter begins by describing in detail the process of drawing a simple plot.  [Building a scatterplot](#simple-plot) starts with a simple scatterplot, then [Adding complexity](#complex-plot) makes it more complex by adding a smooth line and facetting.  While working through these examples you will be introduced to all six components of the grammar, which are then defined more precisely in [Components of the layered grammar](#components).
...
* Scale transformation occurs before statistical transformation so that 
  ...
  See [continuous scales](#scale-position) for more details. 
...
The properties of a layer are described in Chapter \@ref(layers) and their uses for data visualisation in Chapter \@ref(toolbox).

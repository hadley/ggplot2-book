...
You can use `ggplot()` without any understanding of the underlying grammar, but you'll never be able to unlock the full power of ggplot.  By learning more about the grammar and its components, you will be able to create a wider range of plots, as well as being able to combine multiple sources of data, and customise to your heart's content.  You may want to skip this chapter in a first reading of the book, returning when you want a deeper understanding of how all the pieces fit together.
...
How are engine size and fuel economy related? We might create a scatterplot of engine displacement and highway mpg with points coloured number of cylinders: 
...
You can create plots like this easily, but what is going on underneath the surface?  How does **ggplot** draw this plot? \index{Scatterplot!principles of}
...
knitr::kable(head(scaled, 10), digits = 3)
```
...
With a simple example under our belts, let's now turn to look at this slightly more complicated:
...
* Scale transformation occurs before statistical transformation so that 
  ...
  See [continuous scales](#ssub:scale-continuous) for more details. 
...
* After the statistics are computed, each scale is trained on every dataset 
  ...
  and this is described more fully in [controlling scales](#sub:controlling-scales).
...
1. and a position adjustment.

The properties of a layer are described in [layers](#cha:layers) and their uses for data visualisation in [toolbox](#cha:toolbox).
...
A coordinate system, or **coord** for short, maps the position of objects onto the plane of the plot. Position is often specified by two coordinates $(x, y)$, but potentially could be three or more (although this is not implemented in **ggplot**). The Cartesian coordinate system is the most common coordinate system for two dimensions, while polar coordinates and various map projections are used less frequently. \index{Coordinate systems!introduction}
...
This grammar is encoded into R data structures in a fairly straightforward way. A plot object is a list with components `data`, `mapping` (the default aesthetic mappings), `layers`, `scales`, `coordinates` and `facet`. The plot object has one other component we haven't discussed yet: the `theme`. This is used to store the plot-specific theme options described in [Polishing](#cha:polishing). \index{ggplot!data structures}

Most of the time you create a plot object and immediately plot it. But you can also save a plot to a variable and manipulate it. Once you have a plot object, there are a few things you can do with it:
...
* Save a cached copy of it to disk, with `saveRDS()`. This saves a complete
  ...
  `readRDS()`. Note that data is stored inside the plot, so that if you change 
  the data outside of the plot, and then redraw a saved plot, it will not be
  updated.  \indexf{saveRDS} \indexf{readRDS}

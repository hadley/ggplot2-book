...
# how to call knitcitations::bibliography() only when necessary?
refs <- bibtex::read.bib("references.bib")
knitr::opts_chunk$set(cache.path = "cache/mastery-", fig.path = "figures/mastery-")
# set these in parent evetually
library("pander")
...
This chapter describes the theoretical basis of **ggplot**: the layered grammar of graphics.  The layered grammar is based on Wilkinson's grammar of graphics `r citep(refs["wilkinson:2006"])`, but adds a number of enhancements that help it to be more expressive and fit seamlessly into the R environment.  The differences between the layered grammar and Wilkinson's grammar are described fully in `r citep(refs["wickham:2008"])`, and a guide for converting between `GPL` (the encoding of the grammar used in `SPSS`) and **ggplot** is included in [translating](#cha:translating).  In this chapter you will learn a little bit about each component of the grammar and how they all fit together.  The next chapters discuss the components in more detail, and provide more examples of how you can use them in practice.

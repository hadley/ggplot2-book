...
xtable(mpg[1:10, -6], caption = "The first 10 cars in the `mpg` dataset, included in the **ggplot** package.  `cty` and `hwy` record miles per gallon (mpg) for city and highway driving, respectively, and `displ` is the engine displacement in litres.", label = "tbl:mpg")
```
...
xtable(scatter[1:10, ], caption = "First 10 rows from `mpg` rearranged into the format required for a scatterplot.  This data frame contains all the data to be displayed on the plot.", label = "tbl:mapping")
```

...
It is worth noting that the first layer to this plot maps the `fill` aesthetic in onto a variable in the data. In this instance the `name` variable is a categorical variable, but other kinds of area metadata can be added to the plot (e.g., shading to indicate the unemployment level in each state) using the same mechanism.
...
This approach can be extended to handle point metadata more generally. For example if the oz_capitals data set included an additional variable specifying the number of electorates within each metropolitan area, we could map the `size` aesthetic for the points to it. 



## Map projections {#mapproj}

In ths section I duscuss how the `coord_sf()` function sets the coordinates for the plot. The geometry column is usually associated with a coordinate reference system (CRS), stored internally as one of the attributes of the column. Because this information is usually available, the default behaviour of `coord_sf()` is to extract the CRS from the sf geometry column and use that to construct the coordinate system for the plot.

...
All 14 files are constructed from images taken by the Himawari-8 geostationary satellite that is operated by the Japan Meteorological Agency which is able to take images across 13 distinct bands. The images released by the Australian BOM include data on the visible spectrum (channel 3) and the infrared spectrum (channel 13):
...
The first thing to note is that `geom_stars()` requires the `data` argument to be a stars object, and its job is to take the stars object and render it using `geom_raster()` (the default) or `geom_sf()`. The choice of geom can be controlled by the user: you can set `sf = TRUE` to use `geom_sf()`, but note that rendering will be slow. he only way that `geom_stars()` is able to call `geom_sf()` this way is by converting the pixel map to a collection of polygons that `geom_sf()` understands, which is a much slower process than using `geom_raster()`. 

The second point to make about this image is that the raster data is mapped to the fill aesthetic, and the blue shading in the satellite image above is determined by the ggplot2 scale, not the image itself. That is, although `sat_vis` contains three bands, the plot above only displays the first one, and the raw data values (which range from 0 to 255) are mapped onto the default blue palette that ggplot2 uses for continuous data. To see what the image file "really" looks like we can separate the bands using `facet_wrap()`:
...
One limitation to displaying only the raw image is that it is not easy to work out where the relevant landmasses are, and we may wish to overlay the satellite data with the `oz_states` vector map to show the outlines of Australian political entities. However, some care is required in doing so. To see why, notice that the two data sources are associated with different coordinate reference systems:
...
To project the `oz_states` data correctly, the data should be transformed using the `st_transform()` function from the sf package. In the code below, I extract the CRS from the `sat_vis` raster object, and transform the `oz_states` data to use the same system.
...
What we have done here is transform the original `oz_capitals` data, which took the form of a standard data frame (or tibble), in to a properly specified simple features object that has its own CRS. Having done so we can now transform the co-ordinates from the latitude-longitude geometry to the match the geometry of our `sat_vis` data:

...
The first thing to note is that `geom_stars()` requires the `data` argument to be a stars object, and its job is to take the stars object and render it using `geom_raster()` (the default) or `geom_sf()`. The choice of geom can be controlled by the user: you can set `sf = TRUE` to use `geom_sf()`, but note that rendering will be slow. The only way that `geom_stars()` is able to call `geom_sf()` this way is by converting the pixel map to a collection of polygons that `geom_sf()` understands, which is a much slower process than using `geom_raster()`.[^why-sf]

[^why-sf]: In general the `geom_raster()` method is preferable to the `geom_sf()` path because it is so much faster. However, in some cases it may be necessary to set `sf = TRUE` because `geom_raster()` does not understand CRS information.

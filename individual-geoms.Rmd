...
*   `geom_bar(stat = "identity")` makes a __bar chart__. We need 
    ...
    (so is essentially a 1d geom, see [distributions](#distributions). 
...
*   `geom_line()` makes a __line plot__.  The `group` aesthetic determines 
    which observations are connected; see [grouping](#grouping) for more 
...
*   `geom_polygon()` draws polygons, which are filled paths. Each vertex of the 
    ...
    [Drawing maps](#maps) illustrates this concept in more detail for map 

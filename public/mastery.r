...
# More complicated plots don't have their own names.
# This plot takes Figure~\ref{fig:mpg} and adds a
# regression line to each group.  What would you call
# this plot?
qplot(displ, hwy, data=mpg, colour=factor(cyl)) + 
geom_smooth(data= subset(mpg, cyl != 5), method="lm")

# A more complex plot with facets and multiple
# layers.
qplot(displ, hwy, data=mpg, facets = . ~ year) + geom_smooth()

# Examples of legends from four different scales.
# From left to right: continuous variable mapped to
# size, and to colour, discrete variable mapped to
# shape, and to colour.  The ordering of scales seems
# upside-down, but this matches the labelling of the
# $y$-axis: small values occur at the bottom.
x <- 1:10
...
# coordinate systems: Cartesian, semi-log and polar.
# The polar coordinate system illustrates the
# difficulties associated with non-Cartesian

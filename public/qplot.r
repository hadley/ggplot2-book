...
# ----------------------------
# Mapping colour (left), size (middle) and shape (right) of points to
# quality of cut.qplot(carat, price, data=dsmall, colour=cut)
...
# \code{formula = y ~ s(x, bs="cr")}library(mgcv)
...
# ----------------------------
# Like the smooth geom, you can modify the \code{formula} argument to
# control the functional form of the relation.  ({\sc left}) A natural
# spline with five degrees of freedom, {\tt formula = y ~ ns(x, 5)} and
# ({\sc right}) a linear model with break points at 0.5, 1, and 1.5
# carats, {\tt formula = y ~ x + I(x > 0.5) + I(x > 1) + I(x > 1.5))}qplot(carat, price, data=dlittle, geom=c("point", "quantile"), formula=y~ns(x, 5))
...
# seq(0.05, 0.95, 0.1)}qplot(carat, price, data=dlittle, geom=c("point", "quantile"), quantiles=seq(0.05,0.95, 0.1))

# ----------------------------
# Scatterplot of price vs.\ carat supplemented with contours of a 2d
# density estimate ({\tt geom=c("point", "density2d")}).  Most diamonds
# are small and cheap.qplot(carat, price, data=diamonds, geom=c("point","density2d"))
...
# there is little change in the middle half of the distribution.qplot(color, price/carat, data=diamonds, geom="jitter")
...
# lies.  However, the boxplot still does much better.qplot(color, price/carat, data=diamonds, geom="jitter", colour=I(alpha("black", 1/5)))
...
# "histogram"} and (right) \code{geom = "density"}qplot(carat, data=diamonds, geom="histogram")
...
# Only diamonds with carats between 0 and 3 shown.qplot(carat, data=diamonds, geom="histogram", binwidth=1, xlim=c(0,3))
...
# overlaid and (right) histograms are stacked.qplot(carat, data=diamonds, geom="density", colour=color, size = I(1.5))
qplot(carat, data=diamonds, geom="histogram", fill=color)

# ----------------------------
# Bar charts of diamond colour.  The (left) shows counts, and the
# (right) is weighted by \code{weight = carat} to show the total weight
# of diamonds of each colour.qplot(color, data=diamonds, geom="bar")
...
# number of weeks unemployed.  Plots created with {\tt geom="line"}.qplot(date, unemploy/pop, data=economics, geom="line")
...
# within each colour.  Facets created with \code{facets = colour ~ .}qplot(carat, data=diamonds, facets=color ~ ., geom="histogram", binwidth=0.1, xlim=c(0, 3))

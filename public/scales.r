...
# The difference between breaks and limits.  (Left)
# default plot with {\tt limits = c(4, 8), breaks =
# 4:8}, (middle) {\tt breaks = c(5.5,6.5)} and
# (right) {\tt limits = c(5.5,6.5)}.  The effect on
# the x axis (top) and colour legend (bottom)
...
# A scatterplot of diamond price vs.\ carat
# illustrating the difference between log
# transforming the scale (left) and log transforming
# the data (right).  The plots are identical, but the
...
# Gradient colour scales using perceptually
# well-formed palettes produced by the \code{vcd}
...
# A plot of R colours in Luv space.  A legend is
# unnecessary, because the colour of the points
# represents itself: the data and aesthetic spaces
# are the same.
x <- colors()
...
# Legends produced by geom: point, line, point and
# line, and bar.

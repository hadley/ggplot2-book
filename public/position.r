...
# A dotplot showing the range of city gas mileage for
# each model of car. (Left) Models ordered by average
# mpg, and (right) faceted by manufacturer with
# \code{scales="free_y"} and \code{space = "free"}.
# The {\tt strip.text.y} theme setting has been used
# to rotate the facet labels.
mpg3 <- within(mpg2, {
...
# For nested data, there is a clear advantage to
# faceting (top and middle) compared to dodging
# (bottom), because it is possible to carefully
# control and label the facets.  For this example,
...
# Three ways of breaking a continuous variable into
# discrete bins.  From top to bottom: bins of length
# one, six bins of equal length, six bins containing
...
# How coordinate transformations work: converting a
# line in Cartesian coordinates to a line in polar
# coordinates.  The original x position is converted
# to radius, and the y position to angle.

...
<!-- HW: again needs a bit of an overview -->
...
<!-- HW: I think you could probably skip distiller here; most people will be better served by viridis -->
...
Every scale is associated with a guide that displays the relationship between the aesthetic and the data. For position scales, the axes serve this function. For colour scales this role is played by the legend, which can be customised by setting the `guide` argument to the corresponding scale function. For continuous colour scales, the default legend takes the form of a "colour bar" displaying a continuous gradient of colours:
...
<!-- HW: same thing about switching to guide() here -->
...
This task can also be accomplished using the `guides()` function, analogous to the `labs()` and `lims()` helper functions for labels and scale limits. These are equivalent plots:
...
base + 
  guides(
    colour = guide_colourbar(reverse = TRUE)
  )
```
...
Each plot makes sense on its own, but visual comparison between the two is difficult. The axis limits are different, and because only regular, premium and diesel fuels are represented in the 1998 data the colours are mapped inconsistently. To ensure a consistent mapping for the colour aesthetic, we can set the `limits` argument:


```{r}
base_99 + scale_color_discrete(limits = c("c", "d", "e", "p", "r"))
base_08 + scale_color_discrete(limits = c("c", "d", "e", "p", "r"))
```

Remember because the fuel variable `fl` is discrete, the limits for the colour aesthetic are a vector of possible values rather than two end points. Additionally, these plots are a little unsatisfying because the colour aesthetic is mapped consistently but the position aesthetics are not. In this situation the `lims()` function, discussed earlier in Section \@ref(position-continuous-limits), can be very useful. It takes name-value pairs as input, where the name specifies the aesthetic and the value specifies the limits:

<!-- HW: start with lims then switch to scale when you use breaks + labels? -->
...
One potential limitation is that while setting the scale limits does ensure that colours are mapped identically in both plots, it also means that the plot for the 1999 data displays labels for all five fuel types, despite the fact that ethanol and compressed natural gas fuels were not in use at that time. To fix this, we can use the `breaks` argument to ensure that only those fuel types that appear in the data are shown in the legend:
...
These two versions of the 1999 plot display the same labels in the legend, but only the one on the right uses a colour mapping that is consistent with the 2008 plot. 

Finally, we can use the `labels` argument to assign useful labels to a discrete colour scale. This argument behaves in a similar fashion to the corresponding argument for other scale types, discussed in Sections \@ref(position-continuous-labels), \@ref(scale-labels), and \@ref(colour-continuous-limits):
...
<!-- HW: I like this here! Nice foreshadowing :) -->

...
## Adding Text Labels to Points {#sec:labelling}
...
## Building Custom Annotations {#sec:annotations}
\index{Annotation} 
...
## Replacing a Legend with Text Annotation

There are many other ways to customise your annotation in ggplot2, particularly if you look at the broader ecosystem. As an example of this, one common annotation task is to use text labels instead of a legend or to highlight groups of points in a plot. This usually makes the plot easier to read because it puts the labels closer to the data. The [directlabels](https://github.com/tdhock/directlabels) package, by Toby Dylan Hocking, provides a number of tools to make this easier: \index{directlabels}
...
Another take on this idea comes from the ggforce package by Thomas Lin Pederson <https://github.com/thomasp85/ggforce>. The ggforce package contains a lot of useful tools to extend ggplot2 functionality, including functions such as `ggforce::geom_mark_ellipse()` that overlays a plot with circular "highlight" marks. For example:
...
* The gghighlight package by Hiroaki Yutani <https://github.com/yutannihilation/gghighlight> is useful for highlighting points or lines (or indeed a variety of different geoms) within a plot. This can be very helpful for longitudinal data: 

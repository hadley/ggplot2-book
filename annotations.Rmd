...
the data displayed. Conceptually, an annotation supplies *metadata* for the plot,
but in practice metadata is just another form of data so the annotation tools in 
ggplot2 are much the same as any other tool. This chapter is divided into two broad
sections; one focusing text labels, and the other covering other kinds of annotation.
\index{Metadata}
...
While labelling data with  `geom_text()` is an important kind of annotation, it is not the only method. The ggplot2 package provides several other tools to annotate plots. You can use:

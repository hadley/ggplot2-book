...
To learn more about how `labs()` is related to scales in ggplot2, see Section \@ref(scale-names).
...
*   The font size is controlled by the `size` aesthetic. Unlike most tools, 
    ...
    multiply by 25.4 / 72.27). \index{Font!size}
...
*   Often you want to label existing points on the plot, but you don't want the
    ...
    (Note that I manually tweaked the y-axis limits to give the labels a little extra room.)
...
One useful way to annotate this plot is to use shading to indicate which president was in power at the time. To do this, we use `geom_rect()` to introduce shading, `geom_vline()` to introduce separators, `geom_text()` to add labels, and then use `geom_line()` to overlay the data on top of these background elements:

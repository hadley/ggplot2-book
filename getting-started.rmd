...
* About the `mpg` dataset included with ggplot2, [mpg](#fuel-economy-data).
...
* How to display additional categorical variables in a plot using
  small multiples created by facetting, [facetting](#qplot-facetting).

* A variety of different geoms that you can use to create different
  types of plots, [geoms](#plot-geoms).

* How to modify the axes, [axes](#axes).

* Things you can do with a plot object other than display it, like 
  save it to disk, [output](#output).
...
You might wonder when to use facetting and when to use aesthetics. You'll learn more about the relative advantages and disadvantages of each in [grouping vs. facetting](#group-vs-facet).
...
(You'll learn how to fix the labels in [axis labels](#theme-axis)).
...
*   Render it on screen with `print()`. This happens automatically when
  ...
  * Save it to disk with `ggsave()`, described in [saving your output](#saving).

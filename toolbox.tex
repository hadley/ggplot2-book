...
\begin{itemize}
  ...
  \item Dealing with overplotting, \secref{sec:overplotting}, is a challenge with large datasets.
...
\input{_include/9664c31fd075432a7f0e3c9811c141d5.tex}
% END
...
\input{_include/5c825ea5b0147d9090070b7f37b66465.tex}
% END
...
\input{_include/47292cfa24ad78a1f673ee74bf19e757.tex}
% END
...
\begin{itemize}
  ...
  \input{_include/61e2413e7f23768184c56bcb4f41e3a3.tex}  
  % END
  ...
  \input{_include/eaa1e60d12d86b49c7b0f9fb81a1a0bf.tex}  
  % END
  ...
  \input{_include/d9c18f518e43a8fc722faf41986c7b8b.tex}  
  % END
...
\begin{itemize}
  ...
  \input{_include/7835522f46852297668f2bca836dd6a4.tex}  
  % END
  ...
  \input{_include/cf8b32e956037e3292c423b4a9f53980.tex}  
  % END
  ...
  \input{_include/6049cf31c2f54890b7fee5514122cdee.tex}  
  % END
...
\begin{itemize}
  ...
  \input{_include/b5dafbc3c90f6ff0583c50e207c2f485.tex}  
  % END
  ...
  \input{_include/bb5c8a80769651d96a5db46f12148b46.tex}  
  % END
...
The next example shows how we can use \f{fortify} to get map objects into form that's useful for \ggplot.   This is useful as base method to build more convenient functionality on top of, but it's not terrible appealing as a user interface: the user must know to supply \code{plot = F} and \code{fill = T} to the plot function, and must remember the appropriate mappings.  The results are shown in Figure~\ref{fig:fortify-map}.

% FIGLISTING
%   LABEL: fortify-map
%   CAPTION: Left, a map of borders of the USA, and right, individual state
%   borders.  Both of these were produced with data from the maps package, 
%   fortified in to a form that works with \ggplot.
% 
...
% }
% 
% qplot(x, y, data = map("usa", plot = F, fill = T), geom = "path", group = group)
% qplot(x, y, data = map("state", plot = F, fill = T), geom = "path", group = group)
\input{_include/772f155fca6d2efcaf34114ca0a179b7.tex}
% END

Lets make a function that makes it easier to add maps to existing plots.  In the previous section you saw the fortify method that will make a data frame we can plot, but let make the interface even easier: we want a function that will add a layer that draws a map of the specified region.  First, we need to identify the arguments to \f{map} that users will want to control: apart from  \code{database} and \code{regions} all of the arguments control the appearance of the plot, something we're not interested in.  (Again the combination of data manipulation and display code makes this function fiddly to reuse).  We will always use \code{geom_polygon} to draw the map, so don't need to make the geom a parameter, but will need to ensure that other arguments get passed on so that we can control the appearance.
...
% }
\input{_include/1de9016f85ba8503be1596b0e4268e97.tex}
% END
...
% FIGLISTING
%   LABEL: borders
%   CAPTION: Example using the new borders function.  Left, all cities with
...
% qplot(long, lat, data = tx_cities) + borders("state", "texas")
\input{_include/102852db80a169eff06ec9faf1d5f8af.tex}
% END
...
You may want to look at RGL, \url{http://rgl.neoscientists.org/about.shtml}, for interactive 3d plots, including surfaces.
...
\input{_include/a59d78b31d0f5e977f9d77040ab265e7.tex}
% END
...
\input{_include/fcf87581fe5b9bd9eceda5522cc86ae7.tex}
% END
...
\input{_include/b6329df6c59555738308ffb95456977d.tex}
% END
...
\input{_include/8c2bc5d1347aeabe77d10151b2153dce.tex}
% END
...
\input{_include/9aae249875285e445500f7f241f89fb7.tex}
% END
...
\input{_include/009ad9ebbe74d1e9f44dd9a2a661c237.tex}
% END
...
\input{_include/0cbe386ec2d2458a0c3e8c9bba7efdca.tex}
% END
...
\input{_include/adf4316e0f9b32891034c4899c3bdefb.tex}
% END
...
% FIGURE
%   LABEL: weight-hist
%   CAPTION: The different between a, \leftc, unweighted and \rightc,
%   weighted histogram.  The unweighted histogram shows number of counties,
%   while the weighted histogram shows population
%
...
\input{_include/edb34f5a65f80e599eac79dbe9bc0187.tex}
% END

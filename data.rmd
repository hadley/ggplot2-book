...
library("tidyr")
library("plyr")
knitr::opts_chunk$set(cache.path = "cache/data-", fig.path = "figures/data-")
...
* `.variables` is a description of the grouping variables used to break up the dataset. This is written like \code{.(var1, var2)}, and to match the plot should contain all the grouping and faceting variables that you've used in the plot.
...
To perform this transformation we will use the `melt()` function from the **reshape** package \citep{wickham:2007b}. Reshape also provides the `cast()` function to flexibly reshape and aggregate data, which you may want to read about yourself. Table~\ref{tbl:melt} gives an example. The `melt()` function has three arguments: \index{Package!reshape} \indexf{melt}
...
```{r plot-lm, out.width="0.4\\linewidth", fig.cap="The output from `plot.lm()` for a simple model."}
...
pander::pandoc.table(head(fortify(mpgmod)), caption = "The output of `fortify(mpgmod)` contains the two variables used in the model (`cty` and `displ`), and the six diagnostic variables described above.")
```

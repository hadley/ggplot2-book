...
There are several other position scales for continuous variables---`scale_x_log10()`, `scale_x_reverse()`, etc---most of which are convenience functions used to provide easy access to common transformations, discussed in in Section \@ref(scale-transformation).
...
Analogous to the `labs()` function used to specify axis labels (Section \@ref(titles)), `lims()` takes name-value pairs as inputs: the argument name is used to specify the aesthetic, and the value is used to specify the scale limits.
...
You'll see an example of `lims()` applied to non-position aesthetics in Section \@ref(colour-discrete-limits).
...
You can learn more about coordinate systems in Section \@ref(cartesian).
To learn more about how "out of bounds" values are handled for continuous and binned scales, see Section \@ref(oob).
...
The following plots illustrate the effect of setting the minor breaks: `r columns(2, 2/3)`
...
See Section \@ref(cartesian) for more details on coordinate systems, and Section \@ref(scale-transformation-extras) if you need to transform something other than a numeric position scale.
...
This section discusses date/time scales for position aesthetics: see Section \@ref(date-colour-scales) for colour and fill aesthetics.
...
We'll discuss date labelling in Section \@ref(date-labels), but for now our focus is on the breaks.
...
The longer form is typically unnecessary, but it can be useful if---as discussed in Section \@ref(position-continuous-breaks)---you wish to specify an `offset`.
...
To illustrate this, we'll define an empty plot with a date scale on the y-axis, and tweak the theme (Chapter \@ref(polishing)) to make the grid lines more visually prominent:
...
An alternative approach is to pass a labelling function to the `labels` argument, in the same way we described in Section \@ref(position-continuous-labels).
...
To illustrate this, we can add a custom annotation (see Section \@ref(custom-annotations)) to the plot:
...
For the most part these behave identically to the corresponding arguments for numeric scales (Section \@ref(numeric-position-scales)), though there are some differences.
...
We can control this with the help of the `guides()` function, which works in a similar way to the `labs()` helper function described in Section \@ref(titles).

...
However, this code is a little unwieldy. Because modifying scale limits is such a common task, ggplot2 provides convenience functions to simplify the code. For position scales the `xlim()` and `ylim()` helper functions inspect their input and then specify the appropriate scale for the x and y axes respectively:
\indexf{xlim}  \indexf{ylim} 

<!-- HW: I wonder if we should just teach lims everywhere? It lines up nicely with labs -->
...
Another option for setting limits is the `lims()` function which takes name-value pairs as input, where the name specifies the aesthetic and the value specifies the limits. This function can be useful when other scale limits need to be set, discussed in Section \@ref(colour-discrete-limits).
...
<!-- HW: Maybe worth showing some non-integer positions? This is mildly important to know because it's how jitter works, and it's what allows the width of bars and boxplots to work. So maybe better to illustrate with bar width? -->

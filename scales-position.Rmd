...
```{r}
base + scale_x_continuous(breaks = c(2000, 4000), labels = c("2k", "4k"))
```
...
base + scale_y_continuous(labels = scales::label_dollar(prefix = "", suffix = "€"))
```
...
base 
base + scale_y_discrete(limits = c("a", "b", "c", "d", "e")) # add category
base + scale_y_discrete(limits = c("d", "c", "a", "b")) # reorder categories
```
...
An alternative way to accomplish the same task is with the `guides()` function which takes name/value pairs as input, where the name specifies the name of the aesthetic and the value is a call to a guide function such as `guide_axis()`. For example, if you want to rotate the axis labels, either of the following will work:
